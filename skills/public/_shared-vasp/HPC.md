...
VASP 相关 skill 统一读取 sandbox 下同一个文件：`/mnt/skills/public/_shared-vasp/profiles.yaml`。

## 一次性配置

```bash
cp /mnt/skills/public/_shared-vasp/config.template.yaml /mnt/skills/public/_shared-vasp/config.yaml
chmod 600 /mnt/skills/public/_shared-vasp/profiles.yaml
chmod 600 /mnt/skills/public/_shared-vasp/config.yaml
```

填入至少一个 profile（local / generic-slurm / scnet-h100 三种类型）。模板 `hpc-profiles.template.yaml` 的 `default_profile` 为 **scnet-h100**，与「VASP 默认走 SCNet」一致；若仍要以本地为主，请把 `default_profile` 改为 `local`。

## 项目级配置（可选）

项目配置文件固定为 `/mnt/skills/public/_shared-vasp/config.yaml`，只需要：

```yaml
profile: sugon-cancon          # 引用 /mnt/skills/public/_shared-vasp/profiles.yaml 里的 profile 名
executor: scnet                # 默认把 VASP 作业提交到 SCNet（非本地）；本地调试可改为 local 或用 CLI --executor local
...
如果省略 `profile:`，使用 `/mnt/skills/public/_shared-vasp/profiles.yaml` 里的 `default_profile`。
...
- `ProfileNotFoundError`：检查 `profile:` 名拼写、`/mnt/skills/public/_shared-vasp/profiles.yaml` 是否存在
