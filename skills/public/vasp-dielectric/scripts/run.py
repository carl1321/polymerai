#!/usr/bin/env python3
"""vasp-dielectric CLI — static dielectric tensor + Born charges via DFPT."""
...
from vasp_skills_lib.inputs.sets import build_dielectric_inputs, resolve_input_files
...
def main() -> int:
    p = argparse.ArgumentParser(prog="vasp-dielectric")
    p.add_argument("poscar", type=Path, help="relaxed CONTCAR/POSCAR")
    p.add_argument("--work-dir", type=Path, default=Path("./eps"))
    ...
    build_dielectric_inputs(structure, args.work_dir, user_incar=user_incar, user_kpoints=user_kpoints)
    ...
    if args.dry_run:
        print(f"DRY RUN: DFPT inputs in {args.work_dir}")
        return 0

    cmd = resolve_vasp_command(config, args.executor)
    submit_script = build_submit_script(config, executor_override=args.executor, job_name="vasp-dielectric")
    submit_and_emit_async(
        ...
        job_name="vasp-dielectric",
        task_kind="vasp_dielectric",
        display_name=args.poscar.stem or "vasp-dielectric",
    ...
    summary = {
        ...
        "task_kind": "vasp_dielectric",
        "note": "DFPT dielectric tensors in OUTCAR after remote completion.",
    }
