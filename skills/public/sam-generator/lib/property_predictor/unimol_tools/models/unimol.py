    ...
    def batch_collate_fn(self, samples):
        ...
        for k in samples[0][0].keys():
            if k == 'src_coord':
                ...
                v = pad_2d([torch.from_numpy(s[0][k]).float() for s in samples], pad_idx=0.0)
                # v = pad_2d([torch.tensor(s[0][k]).float() for s in samples], pad_idx=0.0)
            elif k == 'src_tokens':
                # print("🟡 DEBUG: src_tokens sample types:")
                # for s in samples:
                #     print(f"type: {type(s[0][k])}, value: {s[0][k]}")                
                # v = pad_1d_tokens([torch.tensor(s[0][k]).long() for s in samples], pad_idx=self.padding_idx)
                v = pad_1d_tokens([torch.tensor(s[0][k].tolist(), dtype=torch.long) for s in samples], pad_idx=self.padding_idx)
            batch[k] = v
