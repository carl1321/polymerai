...
help:
	...
	@echo "  make dev             - Start all services in dev mode (no nginx by default; frontend:${DEER_FLOW_FRONTEND_PORT:-18083}, gateway:${DEER_FLOW_GATEWAY_PORT:-18084})"
	@echo "  make dev-with-nginx  - Start dev with Nginx reverse proxy on port 2026"
	...
	@echo "  make start           - Start production mode (no hot-reload; direct ports unless DEER_FLOW_USE_NGINX=1)"
	@echo "  make start-with-nginx - Same as start + Nginx on port 2026 (single URL, like dev-with-nginx)"
...
# Start all services in development mode (default: no nginx; use frontend :18083, gateway :18084)
dev:
	@$(PYTHON) ./scripts/check.py
	@$(RUN_WITH_GIT_BASH) ./scripts/serve.sh --dev

# Start dev with Nginx reverse proxy (single entry http://localhost:2026)
dev-with-nginx:
	@DEER_FLOW_USE_NGINX=1 SKIP_LANGGRAPH_SERVER=1 ./scripts/serve.sh --dev --with-nginx --gateway
...
# Production mode with local Nginx reverse proxy (http://localhost:2026)
start-with-nginx:
	@DEER_FLOW_USE_NGINX=1 SKIP_LANGGRAPH_SERVER=1 ./scripts/serve.sh --prod --with-nginx --gateway
