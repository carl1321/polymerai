...
5. **Start services**:
   ```bash
   make dev                    # Frontend + Gateway on separate ports (see `make help`)
   make dev-with-nginx         # Single entry: http://localhost:2026 (recommended if you want one URL)
   ```

5. **Access**:
   - With `make dev-with-nginx`: http://localhost:2026
   - With `make dev` only: frontend and gateway use the ports printed in the terminal (defaults: `18083` / `18084`; see `DEER_FLOW_FRONTEND_PORT` / `DEER_FLOW_GATEWAY_PORT`).

   **Production-style local run** (no hot-reload): `make start`. To use Nginx on port 2026 like dev-with-nginx, run `make start-with-nginx` (or `DEER_FLOW_USE_NGINX=1 make start`).
...
#### Public agent link (no login)

With **app database** configured (`app_database.url`) and after signing in to the workspace, open **Agents** management and click **公开 (Publish)** on a card to generate a URL like `https://<host>/a/<slug>?token=<secret>`. Visitors can chat with that agent without logging in. The gateway validates the token, blocks thread search/count, thread delete, and stateless `runs/*` paths, and rate-limits by IP+slug. Point **`LANGGRAPH_INTERNAL_URL`** at your LangGraph server (default `http://127.0.0.1:2024`). Nginx configs proxy `/api/public` to the gateway when using unified port `2026`.
...
Thread files are now user-scoped on the host side. New sessions are stored under `.deer-flow/users/{user_id}/threads/{thread_id}/...` (legacy `.deer-flow/threads/{thread_id}` remains readable for migration). Login/auth behavior remains on `/api/auth/*`.
