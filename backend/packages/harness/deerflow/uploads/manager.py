...
def get_uploads_dir_for_owner(thread_id: str, fs_user_id: str | None) -> Path:
    """Return the uploads directory path for a thread (no side effects)."""
    validate_thread_id(thread_id)
    return get_paths().sandbox_uploads_dir(thread_id, user_id=fs_user_id)


def get_uploads_dir(thread_id: str) -> Path:
    """Return the uploads directory path for a thread (no side effects)."""
    return get_uploads_dir_for_owner(thread_id, resolve_fs_user_id_for_thread())


def ensure_uploads_dir_for_owner(thread_id: str, fs_user_id: str | None) -> Path:
    """Return the uploads directory for a thread, creating it if needed."""
    base = get_uploads_dir_for_owner(thread_id, fs_user_id)
