...
class ThreadMetaStore(abc.ABC):
    ...
    @abc.abstractmethod
    async def allows_anonymous_artifact_read(self, thread_id: str) -> bool:
        """True if unauthenticated HTTP clients may read artifacts for this thread.

        Used for GET artifact URLs without a session cookie. Only threads whose
        stored owner is explicitly shared (``NULL`` or the default bucket id)
        return True; missing rows and private owner rows return False.
        """
        pass
