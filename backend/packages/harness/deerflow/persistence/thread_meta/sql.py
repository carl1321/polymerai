...
class ThreadMetaRepository(ThreadMetaStore):
    ...
    async def check_access(self, thread_id: str, user_id: str, *, require_existing: bool = False) -> bool:
        ...
        async with self._sf() as session:
            ...
            if row.user_id is None:
                return True
            # Shared / no-auth bucket: same isolation semantics as NULL owner (everyone may read).
            if str(row.user_id) == DEFAULT_USER_ID:
                return True
    ...
    async def allows_anonymous_artifact_read(self, thread_id: str) -> bool:
        async with self._sf() as session:
            ...
            if row.user_id is None:
                return True
            return str(row.user_id) == DEFAULT_USER_ID
