...
async def init_engine(
    ...
    *,
    ...
    # Tables maintained only via PostgreSQL init scripts (not create_all):
    # - async_tasks — see backend/scripts/sql/async_tasks_pg.sql and init_app_database.py.

    try:
        async with _engine.begin() as conn:
            await conn.run_sync(Base.metadata.create_all)
            if backend == "sqlite":
                runs_columns = {row[1] for row in (await conn.exec_driver_sql("PRAGMA table_info(runs)")).fetchall()}
                if "started_at" not in runs_columns:
                    await conn.exec_driver_sql("ALTER TABLE runs ADD COLUMN started_at DATETIME")
                if "finished_at" not in runs_columns:
                    await conn.exec_driver_sql("ALTER TABLE runs ADD COLUMN finished_at DATETIME")
                if "duration_ms" not in runs_columns:
                    await conn.exec_driver_sql("ALTER TABLE runs ADD COLUMN duration_ms INTEGER")
