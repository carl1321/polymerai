...
def _validate_run_id(run_id: str) -> str:
    """Validate a workflow run ID before using it in filesystem paths."""
    if not _SAFE_RUN_ID_RE.match(run_id):
        raise ValueError(f"Invalid run_id {run_id!r}: only alphanumeric characters, hyphens, and underscores are allowed.")
    return run_id
...
class Paths:
    ...
    def user_thread_dir(self, user_id: str, thread_id: str) -> Path:
        """Per-user thread root: `{base_dir}/users/{user_id}/threads/{thread_id}/`."""
        return self.user_dir(user_id) / "threads" / _validate_thread_id(thread_id)

    def user_workflow_run_dir(self, user_id: str, run_id: str) -> Path:
        """Per-run workflow workspace: `{base_dir}/users/{user_id}/workflow-runs/{run_id}/`."""
        return self.user_dir(user_id) / "workflow-runs" / _validate_run_id(run_id)

    def workflow_run_inputs_dir(self, user_id: str, run_id: str) -> Path:
        return self.user_workflow_run_dir(user_id, run_id) / "inputs"

    def workflow_run_steps_dir(self, user_id: str, run_id: str) -> Path:
        return self.user_workflow_run_dir(user_id, run_id) / "steps"

    def workflow_run_outputs_dir(self, user_id: str, run_id: str) -> Path:
        return self.user_workflow_run_dir(user_id, run_id) / "outputs"
    ...
    def sandbox_shared_dir(self) -> Path:
        """
        Host path for shared sandbox data across all threads.
        Host: `{base_dir}/` (typically `backend/.deer-flow/`)
        Sandbox: `/mnt/user-data/shared/`

        This allows sandbox path `/mnt/user-data/shared/.venv` to reuse
        host path `{base_dir}/.venv` directly.
        """
        return self.base_dir
    ...
    def ensure_thread_dirs(self, thread_id: str, user_id: str | None = None) -> None:
        ...
        for d in [
            ...
            d.chmod(0o777)
        # Shared base dir is managed globally; ensure it exists but do not chmod recursively.
        self.sandbox_shared_dir().mkdir(parents=True, exist_ok=True)

    def ensure_workflow_run_dirs(self, user_id: str, run_id: str) -> Path:
        """Create standard workflow run directories; return run root."""
        root = self.user_workflow_run_dir(user_id, run_id)
        for d in [
            root,
            self.workflow_run_inputs_dir(user_id, run_id),
            self.workflow_run_steps_dir(user_id, run_id),
            self.workflow_run_outputs_dir(user_id, run_id),
        ]:
            d.mkdir(parents=True, exist_ok=True)
            d.chmod(0o777)
        return root
