...
async def run_agent(
    ...
    *,
    ...
    # Callers may store the same dict as ``record.kwargs["config"]`` (e.g. async_task
    # terminal follow-up). Mutating ``config`` in-place would poison listing APIs.
    ...
    if isinstance(md0, dict):
        uid0 = md0.get("user_id")
        if isinstance(uid0, str) and uid0.strip():
            owner_user_id = uid0.strip()
    ...
    try:
        ...
        if owner_user_id:
            runtime_ctx["user_id"] = owner_user_id
        try:
            ...
            if _sf is not None:
                runtime_ctx["async_task_repo"] = AsyncTaskRepository(_sf)
        except Exception:
            logger.debug("async_task_repo not attached to runtime (optional)", exc_info=True)
        if journal is not None:
            runtime_ctx["__run_journal"] = journal
        _install_runtime_context(config, runtime_ctx)
        runtime = Runtime(context=cast(Any, runtime_ctx), store=store)
        cfg0 = config.setdefault("configurable", {})
        cfg0["__pregel_runtime"] = runtime
        if owner_user_id:
            cfg0.setdefault("user_id", owner_user_id)
        ...
        started_at = record.metadata.get("started_at") or record.created_at
        ...
        record.metadata["started_at"] = started_at
        record.metadata["finished_at"] = finished_at
        if duration_ms is not None:
            record.metadata["duration_ms"] = duration_ms
        ...
        if checkpointer is not None and thread_store is not None:
            try:
                ...
                if ckpt_tuple is not None:
                    ...
                    if title:
                        await thread_store.update_display_name(thread_id, title, user_id=None)
            except Exception:
        ...
        if thread_store is not None:
            try:
                ...
                await thread_store.update_status(thread_id, final_status, user_id=None)
            except Exception:
