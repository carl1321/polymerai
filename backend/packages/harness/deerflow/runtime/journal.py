...
def _normalize_chain_outputs(outputs: Any) -> dict[str, Any]:
    """Serialize LangGraph chain outputs for DB storage (never pass Command/list raw)."""
    if outputs is None:
        return {"status": "success"}
    if isinstance(outputs, Command):
        return {"status": "success", **_command_output_summary(outputs)}
    if isinstance(outputs, dict):
        return outputs if outputs else {"status": "success"}
    if isinstance(outputs, list):
        commands = [_command_output_summary(o) for o in outputs if isinstance(o, Command)]
        if commands:
            return {"status": "success", "commands": commands}
        return {"status": "success", "item_count": len(outputs)}
    return {"status": "success", "type": type(outputs).__name__}
...
class RunJournal(BaseCallbackHandler):
    ...
    def on_chain_end(self, outputs: Any, *, run_id: UUID, **kwargs: Any) -> None:
        self._put(
            event_type="run.end",
            category="outputs",
            content=_normalize_chain_outputs(outputs),
    ...
    def _put(self, *, event_type: str, category: str, content: str | dict = "", metadata: dict | None = None) -> None:
        ...
        if self._owner_user_id:
            row["user_id"] = self._owner_user_id
