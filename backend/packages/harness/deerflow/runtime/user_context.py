...
def resolve_fs_user_id_for_thread() -> str:
    """User id for per-user thread paths (uploads/workspace/outputs).

    Inside a LangGraph run, the gateway sets ``configurable["user_id"]`` to the
    authenticated owner (including Casdoor-mapped local ``users.id``). Remote or
    embedded workers may not have HTTP :func:`set_current_user` context; in that
    case :func:`get_effective_user_id` would fall back to ``default`` and break
    isolation. Prefer the run config when present.

    Outside a runnable (e.g. plain FastAPI handler), :func:`get_config` raises
    ``RuntimeError`` and this falls back to :func:`get_effective_user_id`.
    """
    try:
