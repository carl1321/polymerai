...
async def _max_seq_for_thread_row_lock(session: AsyncSession, thread_id: str) -> int | None:
    """Latest seq for *thread_id*, optionally locked for update (PostgreSQL only).

    PostgreSQL rejects ``SELECT max(seq) ... FOR UPDATE`` (aggregate + FOR UPDATE).
    Locking the row with the current max seq serializes writers per thread.
    """
    stmt = select(RunEventRow.seq).where(RunEventRow.thread_id == thread_id).order_by(RunEventRow.seq.desc()).limit(1)
    bind = session.bind
    ...
    async def put(self, *, thread_id, run_id, event_type, category, content="", metadata=None, created_at=None):  # noqa: D401
        ...
        async with self._sf() as session:
            async with session.begin():
                max_seq = await _max_seq_for_thread_row_lock(session, thread_id)
                seq = (max_seq or 0) + 1
                row = RunEventRow(
                    thread_id=thread_id,
                    run_id=run_id,
