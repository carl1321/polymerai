...
_SHARED_VENV_PATH = f"{VIRTUAL_PATH_PREFIX}/shared/.venv"
...
def _get_acp_workspace_host_path(thread_id: str | None = None) -> str | None:
    ...
    if thread_id is not None:
        try:
            ...
            host_path = get_paths().acp_workspace_dir(thread_id, user_id=resolve_fs_user_id_for_thread())
            if host_path.exists():
...
def _thread_virtual_to_actual_mappings(thread_data: ThreadDataState) -> dict[str, str]:
    ...
    if shared:
        mappings[f"{VIRTUAL_PATH_PREFIX}/shared"] = shared
...
def wrap_command_with_shared_venv(command: str) -> str:
    """Prefer shared venv for all bash commands in sandbox.

    We do not fail hard when the venv is missing to preserve backward compatibility.
    """
    return (
        f'if [ -x "{_SHARED_VENV_PATH}/bin/python" ]; then '
        f'export VIRTUAL_ENV="{_SHARED_VENV_PATH}"; '
        f'export PATH="{_SHARED_VENV_PATH}/bin:$PATH"; '
        "fi; "
        'export PYTHONPATH="/mnt/skills/public:${PYTHONPATH:-}"; '
        "export DEERFLOW_SANDBOX=1; "
        f"{command}"
    )
...
def ensure_thread_directories_exist(runtime: Runtime | None) -> None:
    ...
    # Create the standard user-data directories (including shared cache dir)
    import os

    for key in ["workspace_path", "uploads_path", "outputs_path", "shared_path"]:
...
def bash_tool(runtime: Runtime, description: str, command: str) -> str:
    ...
    - Prefer a shared virtual environment in `/mnt/user-data/shared/.venv` (reused across threads).
    ...
    try:
        ...
        command = wrap_command_with_shared_venv(command)
