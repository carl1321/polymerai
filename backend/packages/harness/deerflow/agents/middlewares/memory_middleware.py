...
class MemoryMiddleware(AgentMiddleware[MemoryMiddlewareState]):
    ...
    def after_agent(self, state: MemoryMiddlewareState, runtime: Runtime) -> dict | None:
        ...
        """
        config_data = get_config().get("configurable", {})
        agent_id = config_data.get("agent_id")
        resolved_agent_name = self._agent_name or config_data.get("agent_name")
        config = self._memory_config or get_memory_config()
        if not config.enabled or not is_memory_enabled_for_agent(agent_id=agent_id, agent_name=resolved_agent_name):
            return None
        # Use agent_id as per-agent memory key when available.
        memory_key = agent_id or resolved_agent_name
        if not memory_key:
            logger.debug("No agent memory key in context, skipping memory update")
            return None
