...
async def _thread_has_pending_async_tasks(runtime: Runtime) -> bool:
    """True when this thread has async_tasks awaiting poll/callback (blocks todo nag)."""
    ctx = getattr(runtime, "context", None)
    if not isinstance(ctx, dict):
        return False
    repo = ctx.get("async_task_repo")
    tid = ctx.get("thread_id")
    if repo is None or not tid:
        return False
    try:
        n = await repo.count_active_by_thread(str(tid))
    except Exception:
        return False
    return n > 0
...
class TodoMiddleware(TodoListMiddleware):
    ...
    async def aafter_model(
        ...
        """Async path: skip todo completion nag while async_tasks are in-flight for this thread."""
        if await _thread_has_pending_async_tasks(runtime):
