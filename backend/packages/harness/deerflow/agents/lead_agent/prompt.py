...
def _build_clarification_blocks(prefer_rag_over_clarification: bool) -> tuple[str, str, str]:
    """Build clarification-related prompt fragments."""
    if prefer_rag_over_clarification:
        clarification_thinking = (
            "- **PRIORITY CHECK (MEMORY-FIRST)**: Always check whether the injected `<memory>` already contains enough information to answer first. "
            "Only when memory is missing or clearly insufficient should you call `knowledge_base_search`.\n"
        )
        clarification_section = """
<clarification_system>
**WORKFLOW PRIORITY (MEMORY-FIRST): MEMORY → RETRIEVE → CLARIFY → PLAN → ACT**
1. **FIRST**: Always inspect the injected `<memory>` context and try to answer from memory when it already contains sufficient relevant information.
2. **SECOND**: When memory is missing, stale, or clearly insufficient, call `knowledge_base_search` to retrieve supporting information from linked knowledge bases.
3. **THIRD**: If memory and retrieval are both insufficient (or the request is ambiguous), call `ask_clarification`.
4. **FOURTH**: After clarification (if needed), proceed with planning and execution.

**RULES FOR MEMORY-FIRST AGENTS:**
- ✅ Prefer answering from `<memory>` for stable user preferences, known background facts, and recently confirmed context.
- ✅ Use `knowledge_base_search` when the question needs document-grounded details, policy precision, or facts not present in memory.
- ✅ Prefer `knowledge_base_search` over `web_search` for questions that can be answered from the linked knowledge base.
- ✅ Use `ask_clarification` mainly when the question is fundamentally ambiguous, memory and KB retrieval are insufficient, or the user asks for preferences that cannot be inferred from documents.
- ✅ For dangerous or irreversible actions (deploy, delete, modify production data), you MUST still use `ask_clarification` for confirmation.
- ✅ For scripts and commands that **only** operate on `/mnt/skills` and `/mnt/user-data` paths, assume `bash` and Python work by default; do NOT ask whether the environment supports execution unless a real tool error occurs.
- ❌ Do NOT skip checking memory when memory already contains relevant facts.
- ❌ Do NOT immediately call `ask_clarification` just because some details are missing; FIRST check memory, THEN retrieval.
</clarification_system>"""
...
{clarification_section}{skills_section}
...
def get_agent_soul(agent_name: str | None, db_system_prompt: str | None = None) -> str:
    """Build the <soul> block for the lead agent."""
    if db_system_prompt is not None:
        content = db_system_prompt.strip()
        if content:
            return f"<soul>\n{content}\n</soul>\n"
...
def apply_prompt_template(
    ...
    *,
    ...
    prefer_rag_over_clarification: bool = False,
    db_system_prompt: str | None = None,
    ...
    soul = get_agent_soul(agent_name) if db_system_prompt is None else get_agent_soul(agent_name, db_system_prompt=db_system_prompt)
