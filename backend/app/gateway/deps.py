    ...
    *startup-time* run-events configuration the underlying ``event_store``
    ...
    async with AsyncExitStack() as stack:
        ...
        if sf is not None:
            ...
            app.state.async_task_repo = AsyncTaskRepository(sf)
        else:
            from deerflow.runtime.runs.store.memory import MemoryRunStore

            app.state.run_store = MemoryRunStore()
            app.state.feedback_repo = None
            app.state.async_task_repo = None
        ...
        from app.gateway.async_task_dispatcher import async_task_dispatcher_loop
        ...
        def run_ctx_factory() -> RunContext:
            return RunContext(
        ...
        set_async_task_handles(
            AsyncTaskHandles(
                ...
                run_context_factory=run_ctx_factory,
            )
        )

        stop_dispatch = asyncio.Event()
        dispatch_task = asyncio.create_task(async_task_dispatcher_loop(app, stop_dispatch))
        ...
        try:
            ...
            stop_dispatch.set()
            try:
                await asyncio.wait_for(dispatch_task, timeout=15.0)
            except Exception:
                logger.exception("async_task dispatcher shutdown")
...
async def get_current_user_from_request(request: Request):
    ...
    try:
        from extensions.auth.dependencies import resolve_user_from_request

        ext_user = resolve_user_from_request(request, optional=True)
    ...
    try:
        ...
        ext_user = resolve_bearer_user(access_token, optional=True, request=request)
        if ext_user is not None:
            return ext_user
    except Exception:
        pass

    # Legacy fallback path (local auth repository).
