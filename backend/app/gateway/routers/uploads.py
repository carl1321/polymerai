...
from app.gateway.path_utils import resolve_thread_path_user_id
...
from deerflow.uploads.manager import (
    ...
    delete_file_safe,
    enrich_file_listing,
    ensure_uploads_dir_for_owner,
    get_uploads_dir_for_owner,
    list_files_in_dir,
    normalize_filename,
...
async def upload_files(
    ...
    fs_uid = await resolve_thread_path_user_id(thread_id, request)
    ...
    try:
        uploads_dir = ensure_uploads_dir_for_owner(thread_id, fs_uid)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    sandbox_uploads = get_paths().sandbox_uploads_dir(thread_id, user_id=fs_uid)
    uploaded_files = []
    written_paths = []
...
async def list_uploaded_files(thread_id: str, request: Request) -> dict:
    ...
    fs_uid = await resolve_thread_path_user_id(thread_id, request)
    try:
        uploads_dir = get_uploads_dir_for_owner(thread_id, fs_uid)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
