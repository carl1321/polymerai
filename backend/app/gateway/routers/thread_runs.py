...
def _json_safe_run_dict(d: dict[str, Any] | None) -> dict[str, Any]:
    if not d:
        return {}
    try:
...
def _record_to_response(record: RunRecord) -> RunResponse:
    started_at = record.metadata.get("started_at")
    finished_at = record.metadata.get("finished_at")
    ...
    return RunResponse(
        ...
        metadata=_json_safe_run_dict(record.metadata),
        kwargs=_json_safe_run_dict(record.kwargs),
...
async def list_runs(
    ...
    run_mgr = get_run_manager(request)
    records = await run_mgr.list_by_thread(thread_id, user_id=str(user.id))
...
async def get_run(
    ...
    user = _require_user(current_user, request)
    run_mgr = get_run_manager(request)
    record = await run_mgr.get(run_id, user_id=str(user.id))
