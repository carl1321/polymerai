...
from app.gateway.authz import require_thread_artifact_read
from app.gateway.path_utils import resolve_artifact_host_path, resolve_thread_path_user_id, resolve_thread_virtual_path
...
_ACTIVE_CONTENT_SUFFIXES = frozenset({".html", ".htm", ".xhtml", ".svg"})

MAX_SKILL_ARCHIVE_MEMBER_BYTES = 16 * 1024 * 1024
_SKILL_ARCHIVE_READ_CHUNK_SIZE = 64 * 1024


def _is_active_content(path: Path, mime_type: str | None) -> bool:
    if mime_type in ACTIVE_CONTENT_MIME_TYPES:
        return True
    return path.suffix.lower() in _ACTIVE_CONTENT_SUFFIXES
...
@require_thread_artifact_read
async def get_artifact(
    thread_id: str,
    path: str,
    request: Request,
    download: bool = False,
    preview: Annotated[bool, Query(description="Inline text for HTML preview in the UI")] = False,
) -> Response:
    ...
    if ".skill/" in path:
        ...
        actual_skill_path = resolve_thread_virtual_path(thread_id, skill_file_path, fs_user_id=fs_uid)
        ...
        if preview and _is_active_content(Path(internal_path), mime_type):
            return PlainTextResponse(content=content.decode("utf-8"), media_type=mime_type, headers=cache_headers)

        if download or _is_active_content(Path(internal_path), mime_type):
    ...
    actual_path = resolve_artifact_host_path(thread_id, path, fs_user_id=fs_uid)

    logger.info(
        "Resolving artifact path: thread_id=%s, requested_path=%s, actual_path=%s",
        thread_id,
        path,
        actual_path,
    )
    ...
    if preview and _is_active_content(actual_path, mime_type):
        return PlainTextResponse(content=actual_path.read_text(encoding="utf-8"), media_type=mime_type or "text/html")
    ...
    if _is_active_content(actual_path, mime_type):
        return FileResponse(path=actual_path, filename=actual_path.name, media_type=mime_type, headers=_build_attachment_headers(actual_path.name))
