...
def require_thread_artifact_read[**P, T](func: Callable[P, T]) -> Callable[P, T]:
    """Require authenticated thread read, or anonymous read only for shared threads."""
    ...
    async def wrapper(*args: Any, **kwargs: Any) -> Any:
        ...
        if request is None:
            if "request" in inspect.signature(func).parameters:
                ...
                raise ValueError("require_thread_artifact_read requires 'request' parameter")
            request = kwargs["request"]
        ...
        thread_id = kwargs.get("thread_id")
        if thread_id is None:
            raise ValueError("require_thread_artifact_read requires thread_id")
        ...
        if auth.is_authenticated:
            if not auth.has_permission("threads", "read"):
                raise HTTPException(
                    status_code=403,
                    detail="Permission denied: threads:read",
                )
            allowed = await thread_store.check_access(
                thread_id,
                str(auth.user.id),
            ...
            if not await thread_store.allows_anonymous_artifact_read(thread_id):
                raise HTTPException(
                    status_code=404,
                    detail=f"Thread {thread_id} not found",
                )
