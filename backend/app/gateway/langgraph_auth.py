...
PUBLIC_LANGGRAPH_PROXY_SECRET_HEADER = "X-DeerFlow-Public-LangGraph-Secret"
_PUBLIC_PROXY_SECRET = os.getenv("DEER_FLOW_PUBLIC_LANGGRAPH_PROXY_SECRET", "").strip()


def _check_csrf(request) -> None:
    """CSRF validation disabled — kept for LangGraph auth hook compatibility."""
    return
...
async def authenticate(request):
    ...
    if _PUBLIC_PROXY_SECRET:
        proxied = request.headers.get(PUBLIC_LANGGRAPH_PROXY_SECRET_HEADER)
        if proxied and secrets.compare_digest(proxied, _PUBLIC_PROXY_SECRET):
