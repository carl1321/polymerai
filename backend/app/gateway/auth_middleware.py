...
_PUBLIC_PATH_PREFIXES: tuple[str, ...] = (
    ...
    "/redoc",
    "/openapi.json",
    # Public agent share (slug token validated inside extensions.public_agent.router)
    "/api/public/p/",
)
...
# Extensions auth (`extensions/auth/routes.py`, prefix /api/auth) — anonymous login / OIDC redirect
_PUBLIC_EXTENSION_AUTH_EXACT: frozenset[str] = frozenset(
    {
        "/api/auth/login",
        "/api/auth/public-key",
        "/api/auth/casdoor/enabled",
        "/api/auth/satoken/enabled",
...
def _is_public(path: str) -> bool:
    ...
    if stripped in _PUBLIC_EXACT_PATHS or stripped in _PUBLIC_EXTENSION_AUTH_EXACT:
        return True
    return any(path.startswith(prefix) for prefix in _PUBLIC_PATH_PREFIXES)


def _is_optional_auth_artifact_read(request: Request) -> bool:
    """GET/HEAD /api/threads/{id}/artifacts/... may proceed without a cookie.

    The artifact handler then allows the response only for shared threads
    (metadata owner NULL or default bucket); private threads get 404.
    """
    if request.method not in ("GET", "HEAD"):
        return False
    parts = request.url.path.split("/")
    # ['', 'api', 'threads', thread_id, 'artifacts', ...]
    return len(parts) >= 5 and parts[1] == "api" and parts[2] == "threads" and parts[4] == "artifacts"
...
class AuthMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        # Non-public path: require either session cookie or Bearer token
        auth_header = request.headers.get("authorization", "")
        has_bearer = auth_header.lower().startswith("bearer ")
        if internal_user is None and not request.cookies.get("access_token") and not has_bearer:
            if _is_optional_auth_artifact_read(request):
