...
from extensions._core.workflow.workflow_request import WorkflowConfigRequest, WorkflowEdge, WorkflowNode
...
def format_node_output(output: Any, output_format: str, output_fields: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    """
    根据输出格式和定义的字段处理节点输出
...
def generate_output_schema_prompt(output_format: str, output_fields: list[dict[str, Any]] | None) -> str:
    """
        ...
        if field_name:
            if field_type == "File":
                properties[field_name] = {
                    ...
                    "description": (f'文件字段 {field_name}，相对 workflow work_root 的路径，格式 {{"file": "nodes/<node_id>/POTCAR"}}'),
                }
            else:
                properties[field_name] = {
    ...
    if output_format == "array":
        # 数组格式：[{}]
        schema = {"type": "array", "items": {"type": "object", "properties": properties, "required": list(properties.keys())}}
    else:
        # JSON 格式：{}
...
def _allowed_output_field_names(output_fields: list[dict[str, Any]]) -> set[str]:
    return {str(f["name"]) for f in output_fields if isinstance(f, dict) and f.get("name")}
...
def _restrict_output_payload(data: Any, output_fields: list[dict[str, Any]]) -> Any:
    ...
    if isinstance(data, list):
        return [_restrict_output_payload(item, output_fields) if isinstance(item, dict) else item for item in data]
    return data


def _format_file_ref_into_output_fields(file_ref: Any, output_fields: list[dict[str, Any]]) -> dict[str, Any]:
    from extensions._core.workflow.workflow_output_paths import is_file_ref

    if not is_file_ref(file_ref):
        return {}
    formatted: dict[str, Any] = {}
    file_fields = [f for f in output_fields if isinstance(f, dict) and f.get("name") and _is_file_field_type(str(f.get("type") or ""))]
...
def build_loop_subgraph(
    ...
    loop_body_nodes = [
        ...
        and (
            (hasattr(n.data, "loopId") and n.data.loopId == loop_node_id)
            or (hasattr(n.data, "loop_id") and n.data.loop_id == loop_node_id)
            or (isinstance(n.data, dict) and (n.data.get("loopId") == loop_node_id or n.data.get("loop_id") == loop_node_id))
        )
    ]
    ...
    loop_body_edges = [e for e in edges if e.source in body_node_ids and e.target in body_node_ids]

    # 查找入口节点（循环体内没有上游节点的节点）
    entry_nodes = [n for n in loop_body_nodes if not any(e.target == n.id and e.source not in body_node_ids for e in edges)]

    # 查找出口节点（循环体内没有下游节点的节点）
    exit_nodes = [n for n in loop_body_nodes if not any(e.source == n.id and e.target not in body_node_ids for e in loop_body_edges)]
...
def compile_workflow_to_langgraph(
    ...
    for node in nodes:
        ...
        if node_type == "start":
            ...
            def _start_path_field_keys(ndata: Any) -> frozenset[str]:
                legacy = frozenset({"poscar_path", "poscar", "structure", "contcar", "cif_path"})
                rows: list[Any] = []
                if isinstance(ndata, dict):
                    rows = ndata.get("startInputs") or ndata.get("start_inputs") or []
                elif ndata is not None:
                    rows = getattr(ndata, "startInputs", None) or getattr(ndata, "start_inputs", None) or []
                from_schema = {str(r.get("key")).strip() for r in rows if isinstance(r, dict) and r.get("type") == "path" and r.get("key")}
            ...
            def make_start_node(nid: str, ndata: Any, path_keys: frozenset[str]):
                async def start_node_func(state: WorkflowState):
                    ...
                    user_payload = {k: v for k, v in workflow_inputs.items() if k not in _skip_keys and not str(k).startswith("_")}
                    ...
                    if work_root and user_payload:
                        ...
                        for key, val in user_payload.items():
                            ...
                            elif key in path_keys and isinstance(val, str) and val and not val.startswith("{"):
            ...
            def make_llm_node(nid: str, ndata: Any):
                async def llm_node_func(state: WorkflowState):
                    ...
                    try:
                        ...
                        node_labels = {n.id: node_label_from_data(n.data, n.id) for n in nodes}
                        node_aliases: dict[str, str] = {}
                        for n in nodes:
                            node_aliases[node_label_from_data(n.data, n.id)] = n.id
                            disp = read_node_data_field(n.data, "display_name", "displayName", "label")
                        ...
                        llm_skills_names: list[str] | None = [llm_skill_name] if llm_skill_name else None
                        ...
                        if llm_tools_names:
                            ...
                            node_work_dir = str(Path(str(work_root)) / "nodes" / nid) if work_root else None
                            ...
                            requires_detach = skill_requires_detach(llm_skill_name) if llm_skill_name else False
                            wf_tool_ctx = {
                                ...
                                "allowed_skill_name": llm_skill_name,
                            }
                            ...
                            outputs = format_skill_output(
                                tool_results=tool_texts or ([] if llm_skill_name else [response_content]),
                            ...
                            wants_detach = bool(out_data.get("_awaiting_external") or out_data.get("status") == "submitted")
                            if wants_detach:
                                ...
                                if row and state_manager:
                                    ...
                                    if resume_val:
                                        out_from_resume = resume_val.get("node_output") if isinstance(resume_val, dict) else resume_val
                                    ...
                                    err_detail = f"Detach submit indicated but async_task was not created (envelope={'yes' if envelope else 'no'}, repo={'yes' if repo else 'no'})"
                                    logger.error("Workflow node %s: %s", nid, err_detail)
                                    if state_manager:
                                        state_manager.mark_node_error(nid, err_detail, loop_id=loop_id, iteration=iteration)
                                    raise RuntimeError(f"Workflow stopped: node {nid} detach without async_task")
                            ...
                            result_state: WorkflowState = {"node_outputs": new_node_outputs}
                            if state_manager:
                                state_manager.mark_node_success(nid, node_output_with_inputs, loop_id=loop_id, iteration=iteration)
                            return result_state
                        ...
                        try:
                            ...
                            if _is_molecule_generation_node and isinstance(response_content, str) and response_content:
                                ...
                                debug_path = os.path.join(debug_dir, f"{run_id_for_debug}_{nid}_iter{iter_for_debug}_{ts}.txt")
                                ...
                                logger.warning(f"[LLM_RAW_OUTPUT_SAVED] Molecule generation raw output saved to: {debug_path} (len={len(response_content)}, source={content_source}, model={model_name})")
                                logger.warning(
                                    f"[LLM_RAW_OUTPUT_BEGIN] run_id={run_id_for_debug} node_id={nid} iter={iter_for_debug} source={content_source}\n{response_content}\n[LLM_RAW_OUTPUT_END] run_id={run_id_for_debug} node_id={nid}"
                                )
                        ...
                        try:
                            ...
                            has_molecule_keys = ("id" in field_names or "generation_id" in field_names or "generationId" in field_names) and "smiles" in field_names
                            # 或者检查实际解析出的数据中是否包含这些字段（作为后备检查）
                            if not has_molecule_keys and isinstance(parsed_response, dict):
                                has_id_or_gen_id = "id" in parsed_response or "generation_id" in parsed_response or "generationId" in parsed_response
                            ...
                            if force_array and isinstance(parsed_response, dict):
                                ...
                                logger.debug(f"LLM node {nid} normalized single-object response to array (force_array=True, has_molecule_keys={has_molecule_keys})")
            ...
            def make_tool_node(nid: str, ndata: Any):
                async def tool_node_func(state: WorkflowState):
                    ...
                    try:
                        ...
                        if state_manager:
                            input_data = {"tool_name": tool_name, "params": raw_tool_params}
                            state_manager.mark_node_running(nid, input_data=input_data, loop_id=loop_id, iteration=iteration, timeout_seconds=timeout_seconds)
                        ...
                        for n in nodes:
                            ...
                            disp_t = read_node_data_field(n.data, "display_name", "displayName", "label")
                        ...
                        tool_params = prepare_tool_invoke_params(parsed_params, param_defs, work_root=work_root_str)
                        ...
                        try:
                            ...
                            if isinstance(tool_func, ScriptWorkflowTool) and node_output_dir:
                                ...
                                result = await asyncio.wait_for(tool_func.ainvoke(tool_params), timeout=timeout_seconds)
                            elif hasattr(tool_func, "invoke"):
                                result = await asyncio.wait_for(asyncio.to_thread(tool_func.invoke, tool_params), timeout=timeout_seconds)
                            else:
                                import inspect

                                if inspect.iscoroutinefunction(tool_func):
                                    result = await asyncio.wait_for(tool_func(**tool_params), timeout=timeout_seconds)
                            ...
                            if work_root_tool and isinstance(result, str) and result.strip():
                                ...
                                if not is_file_ref(result):
                                    ...
                                    if not _candidates[0].is_absolute():
                                        _candidates.append(_Path(str(work_root_tool)) / text.lstrip("/"))
                                    for _p in _candidates:
                                        try:
                                            if _p.is_file():
                                                result = to_relative_file_ref(str(work_root_tool), str(_p.resolve()))
                                                break
                                        except OSError:
                                            continue
                        except TimeoutError:
                        ...
                        has_output_schema = isinstance(output_fields, list) and len(output_fields) > 0
            ...
            def make_condition_node(nid: str, ndata: Any):
                async def condition_node_func(state: WorkflowState):
                    ...
                    try:
                        ...
                        if state_manager:
                            input_data = {"condition_expression": raw_condition_expression}
                            state_manager.mark_node_running(nid, input_data=input_data, loop_id=loop_id, iteration=iteration, timeout_seconds=timeout_seconds)
                        ...
                        try:
                            ...
                            result = await asyncio.wait_for(asyncio.to_thread(eval, condition_expression, {"__builtins__": {}}, node_outputs), timeout=timeout_seconds)
                    ...
                    try:
                        ...
                        while iteration < max_iterations:
                            ...
                            async def execute_single_node(node_id: str):
                                ...
                                if body_node_func:
                                    ...
                                    try:
                                        ...
                                        # 执行节点函数（使用超时）
                                        body_result = await asyncio.wait_for(body_node_func(body_state), timeout=timeout_seconds)
                            ...
                            if state_manager:
                                ...
                                for body_node_id, results in iteration_results.items():
                                    if body_node_id != nid:  # 排除循环节点本身
                                        ...
                                        logger.info(
                                            f"Updated iteration {iteration} results for loop body node {body_node_id} to database ({len(current_iteration_outputs)} total iterations, output keys: {list(last_output.keys()) if isinstance(last_output, dict) else 'not a dict'})"
                                        )
                                ...
                                # 使用update_node_output只更新输出，保持running状态
                                state_manager.update_node_output(nid, loop_node_output, loop_id=nid, iteration=iteration)
                            ...
                            if break_conditions:
                                ...
                                logger.info(
                                    f"Loop iteration {iteration}: current_pending from variables: {len(current_pending) if current_pending else 0} items, IDs: {[item.get('id', 'N/A') if isinstance(item, dict) else 'N/A' for item in current_pending]}"
                                )
                                    ...
                                    if source_data is not None:
                                        ...
                                        try:
                                            ...
                                            if isinstance(source_data, list):
                                                ...
                                                for pending_item in current_pending:
                                                    ...
                                                    if matched_data:
                                                        ...
                                                        logger.info(
                                                            f"Loop iteration {iteration}: matching pending_item id={match_id} with source_data, field_path={actual_field_path}, field_value={field_value}, condition={filter_operator} {filter_value}, result={condition_result}"
                                                        )
                                                        ...
                                                        if condition_result:
                                                            ...
                                                            logger.info(
                                                                f"Loop iteration {iteration}: item id={match_id} passed condition (field_value={field_value} {filter_operator} {filter_value}), moved to passed_items with current iteration data"
                                                            )
                                                        else:
                                                            # 对于不满足条件的，也使用当前迭代的最新数据
                                                            pending_items.append(matched_data)
                                                            logger.info(
                                                                f"Loop iteration {iteration}: item id={match_id} did not pass condition (field_value={field_value} {filter_operator} {filter_value}), kept in pending_items with current iteration data"
                                                            )
                                                    else:
                                                        ...
                                                        logger.debug(
                                                            f"Loop iteration {iteration}: source_data items: {[item.get('generation_id') or item.get('generationId') or item.get('id', 'N/A') if isinstance(item, dict) else 'N/A' for item in source_data[:5]]}"
                                                        )
                                            ...
                                            if isinstance(pending_items, list):
                                                ...
                                                logger.info(
                                                    f"Loop iteration {iteration}: set pending_items from filtered data (other type, converted to array): {1 if pending_items else 0} items (only items that do not meet exit condition)"
                                                )
                                            ...
                                            logger.info(
                                                f"Loop iteration {iteration}: verified pending_items in variables: {len(updated_pending)} items (filtered, only items that do not meet exit condition), IDs: {[item.get('id', 'N/A') if isinstance(item, dict) else 'N/A' for item in updated_pending[:5]]}"
                                            )
                                            ...
                                            logger.error(
                                                f"Filter context: current_pending={len(current_pending) if current_pending else 0} items, source_data={len(source_data) if isinstance(source_data, list) else 'not a list'}, filter_field={actual_field_path}, operator={filter_operator}, value={filter_value}"
                                            )
    ...
    for node_id in condition_nodes:
        ...
        if node_edges:
            ...
            builder.add_conditional_edges(node_id, make_condition_router(node_id), condition_map)
