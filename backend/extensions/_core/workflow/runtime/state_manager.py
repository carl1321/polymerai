...
class NodeExecutionStateManager:
    ...
    def mark_node_skipped(self, node_id: str, reason: str | None = None, **kwargs) -> bool:
        """标记节点为跳过"""
        is_new = node_id not in self._global_processed_nodes
    ...
    def mark_node_cancelled(self, node_id: str, reason: str | None = None, **kwargs) -> bool:
        """标记节点为已取消"""
        is_new = node_id not in self._global_processed_nodes
    ...
    def get_node_error(self, node_id: str) -> str | None:
        """获取节点错误"""
...
class DatabaseStateManager(NodeExecutionStateManager):
    ...
    def mark_node_running(
        ...
        try:
            ...
            seq = append_log(
                ...
                "info",
                "node_start",
                payload={"node_id": node_id, "loop_id": loop_id, "iteration": iteration, "inputs": input_data},
                node_id=node_id,
            )
            conn.commit()
        ...
        if node_id in self._node_task_ids:
            ...
            try:
                ...
                seq = append_log(
                    ...
                    "info",
                    "node_end",
                    payload={"node_id": node_id, "status": "success", "loop_id": loop_id, "iteration": iteration, "outputs": outputs, "metrics": metrics},
                    node_id=node_id,
                )
                conn.commit()
        ...
        if node_id in self._node_task_ids:
            ...
            try:
                ...
                append_log(
                    ...
                    "error",
                    log_event,
                    payload={"node_id": node_id, "error": error, "loop_id": loop_id, "iteration": iteration, "is_timeout": is_timeout},
                    node_id=node_id,
                )
                ...
                error_info = {"reason": "node_failed", "node_id": node_id, "error": error, "loop_id": loop_id, "iteration": iteration, "message": f"Node {node_id} failed: {error}"}
                update_run_status(
                    conn,
                    self.run_id,
                    status="failed",
                ...
                append_log(
                    ...
                    "error",
                    "workflow_error",
                    payload={**error_info, "reason": "node_failed", "workflow_stopped": True},
                    node_id=node_id,
                )
    ...
    def get_node_task_status(self, node_id: str) -> dict[str, Any] | None:
        """
        从数据库查询节点任务状态
    ...
    def save_checkpoint(self, checkpoint_data: dict[str, Any], checkpoint_id: str | None = None):
        """
