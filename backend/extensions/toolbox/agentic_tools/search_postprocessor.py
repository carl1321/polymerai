...
class SearchResultPostProcessor:
    ...
    def process_results(self, results: list[dict]) -> list[dict]:
        ...
        for result in results:
            ...
            if "page" == cleaned_result.get("type") and self.min_score_threshold and self.min_score_threshold > 0 and cleaned_result.get("score", 0) < self.min_score_threshold:
                continue
        ...
        sorted_results = sorted(cleaned_results, key=lambda x: x.get("score", 0), reverse=True)

        logger.info(f"Search result post-processing: {len(results)} -> {len(sorted_results)}")
    ...
    def processPage(self, result: dict) -> dict:
        ...
        if "content" in result:
            ...
            if len(cleaned_content) < len(original_content) * 0.8:
                logger.debug(f"Removed base64 images from search content: {result.get('url', 'unknown')}")
        ...
        if "raw_content" in cleaned_result:
            ...
            if len(cleaned_raw_content) < len(original_raw_content) * 0.8:
                logger.debug(f"Removed base64 images from search raw content: {result.get('url', 'unknown')}")
    ...
    def processImage(self, result: dict) -> dict:
        ...
        if "image_url" in cleaned_result and isinstance(cleaned_result["image_url"], str):
            ...
            if "data:image" in cleaned_result["image_url"]:
                ...
                if len(cleaned_image_url) == 0 or not cleaned_image_url.startswith("http"):
                    logger.debug(f"Removed base64 data from image_url and the cleaned_image_url is empty or not start with http, origin image_url: {result.get('image_url', 'unknown')}")
                    return {}
                cleaned_result["image_url"] = cleaned_image_url
                logger.debug(f"Removed base64 data from image_url: {result.get('image_url', 'unknown')}")

        # Truncate very long image descriptions
        if "image_description" in cleaned_result and isinstance(cleaned_result["image_description"], str):
            if self.max_content_length_per_page and len(cleaned_result["image_description"]) > self.max_content_length_per_page:
                cleaned_result["image_description"] = cleaned_result["image_description"][: self.max_content_length_per_page] + "..."
                logger.info(f"Truncated long image description from search result: {result.get('image_url', 'unknown')}")
    ...
    def _truncate_long_content(self, result: dict) -> dict:
        ...
        if "content" in truncated_result:
            ...
            if len(content) > self.max_content_length_per_page:
                truncated_result["content"] = content[: self.max_content_length_per_page] + "..."
                logger.info(f"Truncated long content from search result: {result.get('url', 'unknown')}")
        ...
        if "raw_content" in truncated_result:
            ...
            if len(raw_content) > self.max_content_length_per_page * 2:
                truncated_result["raw_content"] = raw_content[: self.max_content_length_per_page * 2] + "..."
                logger.info(f"Truncated long raw content from search result: {result.get('url', 'unknown')}")
