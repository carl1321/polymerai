...
def phase_diagram_tool(
    ...
    try:
        ...
        return f"[ERROR] Phase diagram dependencies missing: {e}. 请安装 `pymatgen` 后重试。"

    api_key = get_str_env("MP_API_KEY", "")
    if not api_key:
        return "[ERROR] 未找到 Materials Project API 密钥。\n\n请在环境变量或 `conf.yaml` 的 `ENV.MP_API_KEY` 中配置有效的 MP_API_KEY。"
    ...
    if not entries or len(entries) < 2:
        return f"在 Materials Project 中 `{chemical_system}` 体系的有效材料数量过少（{len(entries) if entries else 0} 个），无法构建有意义的相图。"
    ...
    lines.append("")
    lines.append("基于 Materials Project 的 0 K 能量凸包（convex hull）分析，下列化合物位于/高于能量凸包。")
    lines.append("")
    ...
    lines.append("")
    lines.append("> 提示：ΔE above hull 越接近 0，材料在热力学上越可能稳定；通常 ΔE < 0.05–0.1 eV/atom 仍可能在实验中合成为亚稳相。")
