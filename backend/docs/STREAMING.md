...
## 会话续接：Run SSE 与 async_tasks（前端定调）

**会话**由 `thread_id`（及 checkpoint）标识；**流式 token**只发生在某个 **`run_id`** 的 Gateway SSE 上（`sse_consumer` + `MemoryStreamBridge`，支持 `Last-Event-ID` 回放有限窗口）。

- **主路径**：浏览器用 LangGraph SDK 的 `runs.stream` / `runs.joinStream`；断线后 SDK 可用 sessionStorage 中的 `lg:stream:${threadId}` 在挂载时触发 **同 `run_id` 的 `joinStream`**，并传入 **`lastEventId`**（默认从 `-1` 起由服务端解释）。详见 `@langchain/langgraph-sdk` 的 `useStream` / `stream.lgp`。
- **新 run**（例如终态 `_start_terminal_followup` 起的 run）：`Last-Event-ID` 绑在旧 run 上无意义；应对 **`GET /api/threads/{id}/runs`** 取当前 `running`/`pending` 的 `run_id` 再 **`joinStream`**。前端在进房、`visibilitychange`、`online`、以及 **async_tasks GET 之后** 用 **`joinActiveRunIfStaleOrMissing`**：除「无 key / key 与 active 不一致」外，若 **同 `run_id` 但本地流已 idle（`isStreamLoading === false`）** 也会重挂，并对失败做有限次退避重试。
- **`async_tasks`**：任务卡片状态以 **`GET /api/threads/{id}/async_tasks`** 对齐服务端；**不依赖** thread 级 `GET …/async_tasks/stream` 做实时推送（网关仍可 `_publish_update` 给其他消费者）。与业界「后台状态轮询 / 独立通道、模型输出单独 SSE」一致。
- **`on_disconnect`**（`RunCreateRequest` / `RunRecord`）：`cancel` 在客户端 SSE 断开时中止 worker；`continue` 则后台跑完但断连期间事件无人消费。若希望用户切走页面后同一 run 仍可被 **join** 接上，应对该 run 使用 **`continue`**（见 `app/gateway/services.py::sse_consumer` 的 `finally`）。

### 长任务（async_task）链路与「终止会话」用语

- **`thread_id` 全程不变**：超算/技能提交产生 `async_tasks` 行后，轮询与终态续跑都绑定**同一线程**；不存在「换 thread 再续」的隐式步骤。若产品删除 LangGraph thread 或换 `thread_id`，长任务与 follow-up 会失配，应避免。
- **「终止当前会话」应理解为**：用户侧**本轮 Run 的流结束**或**关闭 SSE/离开页面**，**不是**销毁 thread 实体。触发 submit 的 **Run A** 在 agent 跑完该轮后进入终态；**后台轮询在网关 dispatcher 中**，不依赖 Run A 仍为 `running`。
- **终态后续写（Run B）**：`async_task_dispatcher._start_terminal_followup` 在同一 `thread_id` 上 **create 新 `run_id`**，注入合成 user 文案后 `run_agent`，从而更新 checkpoint。前端必须在 **`GET /api/threads/{id}/runs` 发现 `running`/`pending`** 后 **`joinStream`**，否则只有任务卡片更新、对话区无 token 流。
- **触发 join 与拉任务列表解耦**：卡片状态以 `GET …/async_tasks`（及可选 thread 级 `async_tasks/stream`）为准；**接上模型输出**只依赖 **Run SSE + join**。前端在进房、`visibilitychange`、`online`、**停留该线程时按间隔（默认 45s，标签页 visible；`NEXT_PUBLIC_ASYNC_TASK_UI_POLL_MS`，0 关闭）** 以及 **每次 async_tasks GET 合并之后** 都会尝试 `joinActiveRunIfStaleOrMissing`（含「同 `run_id` 但本地已失流」时的重挂），见 `frontend/src/core/threads/join-active-run.ts` 与 `hooks.ts`。
...
| 活跃 run attach + 退避 join | `frontend/src/core/threads/join-active-run.ts` |
