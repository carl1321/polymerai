...
def test_upload_files_writes_thread_storage_and_skips_local_sandbox_sync(tmp_path):
    ...
    with (
        patch.object(uploads, "resolve_thread_path_user_id", AsyncMock(return_value="u1")),
        patch.object(uploads, "ensure_uploads_dir_for_owner", return_value=thread_uploads_dir),
...
def test_delete_uploaded_file_removes_generated_markdown_companion(tmp_path):
    ...
    with (
        patch.object(uploads, "resolve_thread_path_user_id", AsyncMock(return_value="u1")),
        patch.object(uploads, "get_uploads_dir_for_owner", return_value=thread_uploads_dir),
    ):
