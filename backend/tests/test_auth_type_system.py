...
def test_csrf_middleware_allows_post_without_token():
    """POST to protected endpoint without CSRF token is allowed (CSRF disabled)."""
    client = TestClient(_make_csrf_app())
    resp = client.post("/api/v1/test/protected")
    assert resp.status_code == 200


def test_csrf_middleware_allows_post_with_mismatched_token():
    """POST with mismatched CSRF cookie/header is allowed (CSRF disabled)."""
    client = TestClient(_make_csrf_app())
