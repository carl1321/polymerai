...
def _attach_thread_delete_route_deps(app, *, owned_thread_id: str | None = None, owner_user_id: str | None = None):
    """DELETE /api/threads/{id} reads run_store, run_event_store, run_manager from app.state."""
    app.state.run_store = MagicMock()
    app.state.run_store.list_by_thread = AsyncMock(return_value=[])
    app.state.run_store.delete = AsyncMock()
    app.state.run_event_store = MagicMock()
    app.state.run_event_store.delete_by_thread = AsyncMock()
    app.state.run_manager = MagicMock()
    app.state.run_manager.cleanup = AsyncMock()
    if owned_thread_id is not None and owner_user_id is not None:

        async def _get(tid: str, user_id=None):
            if tid == owned_thread_id:
                return {"user_id": owner_user_id}
            return None

        app.state.thread_store.get = AsyncMock(side_effect=_get)
    else:
...
def test_delete_thread_route_cleans_thread_directory(tmp_path):
    ...
    paths = Paths(tmp_path)
    uid_str = str(owner_uid)
    thread_dir = paths.thread_dir("thread-route", user_id=uid_str)
    paths.sandbox_work_dir("thread-route", user_id=uid_str).mkdir(parents=True, exist_ok=True)
    (paths.sandbox_work_dir("thread-route", user_id=uid_str) / "notes.txt").write_text("hello", encoding="utf-8")

    app = make_authed_test_app(user_factory=user_factory)
    app.include_router(threads.router)
    _attach_thread_delete_route_deps(app, owned_thread_id="thread-route", owner_user_id=uid_str)
...
def test_delete_thread_route_rejects_invalid_thread_id(tmp_path):
    ...
    app = make_authed_test_app()
    app.include_router(threads.router)
    _attach_thread_delete_route_deps(app)
...
def test_delete_thread_route_returns_404_when_thread_unknown(tmp_path):
    paths = Paths(tmp_path)
...
def test_search_threads_reads_from_thread_store_and_maps_title():
    app = make_authed_test_app()
    app.include_router(threads.router)

    thread_store = MagicMock()
    thread_store.search = AsyncMock(
        return_value=[
            {
                "thread_id": "t1",
                "status": "idle",
    ...
    with (
        patch("app.gateway.routers.threads.get_thread_store", return_value=thread_store),
        patch("app.gateway.routers.threads._user_id_str", return_value="u1"),
    ):
        with TestClient(app) as client:
            resp = client.post("/api/threads/search", json={"limit": 10, "offset": 0})

    assert resp.status_code == 200
    body = resp.json()
    assert body[0]["thread_id"] == "t1"
    assert body[0]["values"]["title"] == "My Title"


def test_create_thread_writes_thread_store_first():
    app = make_authed_test_app()
    app.include_router(threads.router)

    thread_store = MagicMock()
    thread_store.get = AsyncMock(return_value=None)
    thread_store.create = AsyncMock(return_value={"thread_id": "t-new"})
    app.state.store = None
    app.state.checkpointer = MagicMock()
    app.state.checkpointer.aput = AsyncMock(return_value=None)
    ...
    with (
        ...
        with TestClient(app) as client:
            resp = client.post("/api/threads", json={"thread_id": "t-new", "metadata": {}})

    assert resp.status_code == 200
    thread_store.create.assert_awaited_once()


def test_update_thread_state_syncs_title_to_thread_store_display_name():
    """Rename from the list uses POST /state; threads_meta.display_name must update too."""
    fixed_uid = uuid4()
    ...
    uid_str = str(fixed_uid)
    thread_store = MagicMock()
    thread_store.check_access = AsyncMock(return_value=True)
    thread_store.allows_anonymous_artifact_read = AsyncMock(return_value=True)
    thread_store.update_display_name = AsyncMock()
    app.state.thread_store = thread_store

    ckpt = {"channel_values": {"title": "Old"}}
    tup = SimpleNamespace(
        checkpoint=ckpt,
        metadata={"user_id": uid_str},
        config={"configurable": {"thread_id": "t-rename", "user_id": uid_str}},
    )

    checkpointer = MagicMock()
    checkpointer.aget_tuple = AsyncMock(return_value=tup)
    checkpointer.aput = AsyncMock(return_value={"configurable": {"checkpoint_id": "newck"}})

    store_item = MagicMock()
    store_item.value = {"metadata": {"user_id": uid_str}, "values": {}}

    store = MagicMock()
    store.aget = AsyncMock(return_value=store_item)
    ...
    with TestClient(app) as client:
        resp = client.post(
            "/api/threads/t-rename/state",
            json={"values": {"title": "New Title"}},
        )

    assert resp.status_code == 200
    thread_store.update_display_name.assert_awaited_once_with(
        "t-rename",
        "New Title",
        user_id=uid_str,
    )
