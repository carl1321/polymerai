...
def test_get_artifact_reads_utf8_text_file_on_windows_locale(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(artifacts_router, "resolve_artifact_host_path", lambda *_a, **_kw: artifact_path)
...
def test_get_artifact_forces_download_for_active_content_in_skill_archive(tmp_path, monkeypatch, filename: str, content: str) -> None:
    ...
    monkeypatch.setattr(artifacts_router, "resolve_thread_virtual_path", lambda *_a, **_kw: skill_path)
...
def test_get_artifact_preview_returns_inline_html(tmp_path, monkeypatch) -> None:
    artifact_path = tmp_path / "view.html"
    artifact_path.write_text("<html><body>preview</body></html>", encoding="utf-8")
    ...
    with TestClient(app) as client:
        response = client.get("/api/threads/thread-1/artifacts/mnt/user-data/outputs/view.html?preview=true")

    assert response.status_code == 200
    assert response.text == "<html><body>preview</body></html>"
