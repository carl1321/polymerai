...
class TestToolCallbacks:
    ...
    @pytest.mark.anyio
    async def test_chain_end_serializes_command_list(self, journal_setup):
        """on_chain_end must not pass raw Command objects into the event store."""
        from langchain_core.messages import AIMessage
        from langgraph.types import Command

        j, store = journal_setup
        cmd = Command(update={"messages": [AIMessage(content="done")]})
        j.on_chain_end([cmd], run_id=uuid4())
        await j.flush()
        events = await store.list_events("t1", "r1")
        end_events = [e for e in events if e["event_type"] == "run.end"]
        assert len(end_events) == 1
        content = end_events[0]["content"]
