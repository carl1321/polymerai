...
def test_csrf_post_missing_token_proceeds_to_jwt():
    """POST without CSRF token proceeds to JWT validation (CSRF disabled)."""
    with pytest.raises(Auth.exceptions.HTTPException) as exc:
...
def test_csrf_post_mismatched_token_proceeds_to_jwt():
    """POST with mismatched CSRF tokens proceeds to JWT (CSRF disabled)."""
    with pytest.raises(Auth.exceptions.HTTPException) as exc:
        asyncio.run(
            authenticate(
                _req(
...
def test_csrf_put_proceeds_to_jwt():
    """PUT proceeds to JWT validation (CSRF disabled)."""
    with pytest.raises(Auth.exceptions.HTTPException) as exc:
        asyncio.run(authenticate(_req(method="PUT", cookies={"access_token": "jwt"})))
    assert exc.value.status_code == 401


def test_csrf_delete_proceeds_to_jwt():
    """DELETE proceeds to JWT validation (CSRF disabled)."""
    with pytest.raises(Auth.exceptions.HTTPException) as exc:
        asyncio.run(authenticate(_req(method="DELETE", cookies={"access_token": "jwt"})))
    assert exc.value.status_code == 401


def test_public_proxy_secret_bypasses_jwt(monkeypatch):
    """Gateway-validated public share proxy may authenticate with a shared secret (optional env)."""
    import app.gateway.langgraph_auth as lg_mod
    from deerflow.runtime.user_context import DEFAULT_USER_ID

    monkeypatch.setattr(lg_mod, "_PUBLIC_PROXY_SECRET", "shared-secret")
    uid = asyncio.run(
        lg_mod.authenticate(
            _req(
                method="POST",
                ...
                headers={
                    "x-csrf-token": "t",
                    lg_mod.PUBLIC_LANGGRAPH_PROXY_SECRET_HEADER: "shared-secret",
                },
            )
        )
    )
