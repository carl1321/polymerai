...
@pytest.mark.anyio
@pytest.mark.no_auto_user
async def test_check_access_default_owner_shared(store):
    """Threads owned by the default bucket are readable by any user."""
    with _as_user(USER_A):
        await store.create("t-default", user_id=DEFAULT_USER_ID)
    assert await store.check_access("t-default", USER_B.id) is True
