...
class TestMigrateThreadDirs:
    ...
    def test_unowned_thread_is_reported_and_not_migrated(self, base_dir: Path, paths: Paths):
        legacy = base_dir / "threads" / "t2" / "user-data" / "workspace"
        legacy.mkdir(parents=True)

        from scripts.migrate_user_isolation import migrate_thread_dirs

        report = migrate_thread_dirs(paths, thread_owner_map={})
        assert report == [{"thread_id": "t2", "user_id": None, "action": "missing_owner"}]
        assert (base_dir / "threads" / "t2").exists()
    ...
    def test_cleans_up_empty_legacy_dir(self, base_dir: Path, paths: Paths):
        ...
        assert (base_dir / "threads").exists()
