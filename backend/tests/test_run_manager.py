...
@pytest.mark.anyio
async def test_list_by_thread_merges_persisted_datetime_timestamps(manager: RunManager):
    """Persisted rows may use datetime for created_at; merge + sort must not raise."""
    ...
    store = AsyncMock()
    store.list_by_thread = AsyncMock(return_value=[persisted_row])
    mgr = RunManager(store=store)

    mem = await mgr.create("thread-1", "lead_agent")

    runs = await mgr.list_by_thread("thread-1", user_id=None)
    assert len(runs) == 2
    by_id = {r.run_id: r for r in runs}
    assert by_id["persisted-1"].created_at.startswith("2026-01-02T12:00:00")
    assert by_id[mem.run_id].run_id == mem.run_id
