...
def test_build_run_config_with_context():
    ...
    # Mirrored configurable (thread_id + routing keys) coexists with context.
    assert config.get("configurable", {}).get("thread_id") == "thread-1"
    assert config["recursion_limit"] == 100
...
def _app_cfg_casdoor(enabled: bool):
    class Casdoor:
        pass
...
def test_enforce_run_user_identity_requires_auth_when_casdoor_enabled():
    from app.gateway.routers.thread_runs import RunCreateRequest
    from app.gateway.services import enforce_run_user_identity

    body = RunCreateRequest(input=None)
    with patch("app.gateway.services.get_app_config", return_value=_app_cfg_casdoor(True)):
        with pytest.raises(HTTPException) as ei:
            enforce_run_user_identity(body, None)
    assert ei.value.status_code == 401


def test_enforce_run_user_identity_strips_context_user_id_when_authenticated():
    from app.gateway.routers.thread_runs import RunCreateRequest
    from app.gateway.services import enforce_run_user_identity
    ...
    body = RunCreateRequest(input=None, context={"user_id": "casdoor-sub-not-uuid", "model_name": "x"})
    with patch("app.gateway.services.get_app_config", return_value=_app_cfg_casdoor(True)):
        enforce_run_user_identity(body, user)
    assert body.context is not None
    assert "user_id" not in body.context
    assert body.context.get("model_name") == "x"


def test_enforce_run_user_identity_strips_matching_context_user_id_too():
    from app.gateway.routers.thread_runs import RunCreateRequest
    from app.gateway.services import enforce_run_user_identity
    from extensions.auth.dependencies import CurrentUser
    ...
    body = RunCreateRequest(input=None, context={"user_id": str(uid)})
    with patch("app.gateway.services.get_app_config", return_value=_app_cfg_casdoor(True)):
        enforce_run_user_identity(body, user)
    assert body.context is not None
    assert "user_id" not in body.context
...
def test_enforce_run_user_identity_unauthenticated_allowed_when_casdoor_off():
    from app.gateway.routers.thread_runs import RunCreateRequest
    from app.gateway.services import enforce_run_user_identity

    body = RunCreateRequest(input=None, context={"user_id": "anything"})
    with patch("app.gateway.services.get_app_config", return_value=_app_cfg_casdoor(False)):
        enforce_run_user_identity(body, None)
