...
import { BellIcon, ChevronUpIcon, Loader2Icon } from "lucide-react";
...
export function MessageList({
  ...
  const answerTiming = (thread.values?.__answer_timing ?? null) as {
    finished_at?: string;
    duration_ms?: number;
  } | null;
      ...
      <ConversationContent className="mx-auto w-full max-w-(--container-width-md) gap-8 pt-8">
        ...
        {groupedMessages.map((group, groupIndex) => {
          ...
          if (group.type === "human" || group.type === "assistant") {
                    ...
                    <MessageListItem
                      ...
                      answerFinishedAt={
                        msg.id === lastAssistantMessageId
                          ? answerTiming?.finished_at
                          : undefined
                      }
                      answerDurationMs={
                        msg.id === lastAssistantMessageId
                          ? answerTiming?.duration_ms
                          : undefined
                      }
                    />
                  );
            ...
            group.type === "system-reminder" ||
            group.type === "async-task-progress"
          ) {
            const message = group.messages[0];
            if (!message || !hasContent(message)) {
              return null;
            ...
            return (
              <div key={group.id} className="w-full">
                <Alert className="border-primary/20 bg-muted/30">
                  <BellIcon aria-hidden className="text-muted-foreground" />
                  <AlertDescription>
                    <MarkdownContent
                      content={extractContentFromMessage(message)}
                      isLoading={false}
