    ...
    buildDecorations(view: EditorView): DecorationSet {
      ...
      tree.iterate({
        enter: (node) => {
          ...
          if (name === "ATXHeading1" || name === "SetextHeading1")
            cls = PROMPT_H1_CLASS;
          else if (name === "ATXHeading2" || name === "SetextHeading2")
            cls = PROMPT_H2_CLASS;
          else if (name === "ATXHeading3" || name === "SetextHeading3")
            cls = PROMPT_H3_CLASS;
          if (cls) {
            decorations.push(
...
export function PromptRichEditor({
  ...
  return (
    <div
      className={cn(
        "prompt-rich-editor overflow-hidden rounded-lg border border-slate-200 dark:border-slate-600",
        className,
      )}
    >
      <CodeMirror
        ...
        className={cn(
          "w-full font-mono text-sm [&_.cm-editor]:min-h-[320px] [&_.cm-scroller]:overflow-auto",
          "[&_.cm-content]:leading-relaxed [&_.cm-focused]:outline-none",
        )}
        style={{ minHeight }}
      />
    </div>
