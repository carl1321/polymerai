...
import {
  ...
  type OutputField,
} from "./components/OutputSchemaEditor";
  ...
  if (Array.isArray(obj)) {
    return (
      <div className="space-y-1">
        ...
        <div className="bg-muted max-h-[200px] overflow-auto rounded-md p-2">
...
function RunResultTab({
  ...
  if (!result && status === "pending") {
    return (
      <div className="text-muted-foreground flex h-full flex-col items-center justify-center p-8">
        <p>暂无运行结果</p>
        <p className="mt-2 text-xs">点击"执行"查看结果</p>
      </div>
    );
  }
      ...
      <div className="space-y-4">
        ...
        {result?.error && (
          <div className="rounded-md border border-red-200 bg-red-50 p-3 dark:border-red-800 dark:bg-red-900/20">
            ...
            <pre className="mt-1 max-h-[100px] overflow-auto text-xs whitespace-pre-wrap text-red-600 dark:text-red-400">
        ...
        <div className="space-y-2">
          <Label className="text-muted-foreground text-xs">迭代次数</Label>
        ...
        <div className="space-y-2">
          <Label className="text-muted-foreground text-xs">
            通过筛选的数据 (passed_items)
          </Label>
          <div className="bg-muted max-h-[300px] overflow-auto rounded-md p-2">
            <pre className="font-mono text-xs whitespace-pre-wrap">
              {formatJSONForDisplay(passedItems)}
            </pre>
          </div>
        </div>
        ...
        <div className="space-y-2">
          <Label className="text-muted-foreground text-xs">
            不满足条件的条目 (pending_items)
          </Label>
          <div className="bg-muted max-h-[300px] overflow-auto rounded-md p-2">
            <pre className="font-mono text-xs whitespace-pre-wrap">
              {formatJSONForDisplay(pendingItems)}
            </pre>
          </div>
        </div>
      </div>
  ...
  if (
    isLoopBodyNode &&
    result?.outputs?.iteration_outputs &&
    Array.isArray(result.outputs.iteration_outputs) &&
    result.outputs.iteration_outputs.length > 0
  ) {
    ...
    const iterations = Object.keys(groupedByIteration)
      .map(Number)
      .sort((a, b) => a - b);
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          <Label className="text-muted-foreground text-xs">迭代执行结果</Label>
                ...
                <AccordionItem key={iter} value={`iteration-${iter}`}>
                  ...
                  <AccordionContent>
                    <div className="space-y-3 pt-2">
                      ...
                      {lastItem.metrics &&
                        Object.keys(lastItem.metrics).length > 0 && (
                          <div className="grid grid-cols-2 gap-2">
                            {Object.entries(lastItem.metrics).map(
                              ([key, value]) => {
  ...
  return (
    <div className="space-y-4">
      ...
      {result?.inputs && (
        <div className="space-y-2">
          <Label className="text-muted-foreground text-xs">输入参数</Label>
          <div className="bg-muted max-h-[200px] overflow-auto rounded-md p-2">
            <pre className="font-mono text-xs whitespace-pre-wrap">
              {formatJSONForDisplay(result.inputs)}
            </pre>
          </div>
        </div>
      )}
      ...
      {result?.outputs != null && result?.outputs !== "" ? (
        <div className="space-y-2">
          <Label className="text-muted-foreground text-xs">输出结果</Label>
          <div className="bg-muted max-h-[300px] overflow-auto rounded-md p-2">
            <pre className="font-mono text-xs whitespace-pre-wrap">
              {formatJSONForDisplay(result.outputs)}
            </pre>
          </div>
        </div>
...
export function NodeConfigPanel({
  ...
  return (
    <div className="border-border bg-card flex h-full min-h-0 w-full min-w-0 flex-col overflow-hidden border-l shadow-lg">
      ...
      >
        ...
        <TabsContent
          value="config"
          className="mt-0 min-h-0 min-w-0 flex-1 overflow-x-hidden overflow-y-auto p-4 data-[state=inactive]:hidden"
        >
...
function StartNodeConfig({ node, nodeData, onUpdate }: NodeConfigProps) {
  ...
  const filesEnabled =
    nodeData.startFiles !== null && nodeData.startFiles !== false;
      ...
      <div>
        ...
        <Input
          ...
          onChange={(e) => {
            ...
            onUpdate(node.id, {
              ...nodeData,
              startInputInfo: e.target.value || undefined,
            });
          }}
        ...
        <div className="space-y-2">
          {startInputs.map((row, idx) => (
            ...
            >
              ...
              <label className="col-span-2 flex cursor-pointer items-center gap-1 text-xs">
                <input
                  type="checkbox"
              ...
              <Button
                ...
                size="icon"
                onClick={() =>
                  persistInputs(startInputs.filter((_, i) => i !== idx))
                }
              >
...
function resolveLlmSkillFromNodeData(
  nodeData: Record<string, unknown>,
): string | null {
  if (nodeData.llmSkill && typeof nodeData.llmSkill === "string")
    return nodeData.llmSkill;
  if (nodeData.llm_skill && typeof nodeData.llm_skill === "string")
    return nodeData.llm_skill;
  const legacy = (nodeData.llmSkills ?? nodeData.llm_skills) as
    | string[]
    | undefined;
...
function LLMNodeConfig({
  ...
  const [llmSkill, setLlmSkill] = useState<string | null>(() =>
    resolveLlmSkillFromNodeData(nodeData),
  );
  const [temperature, setTemperature] = useState(
    nodeData.llmTemperature?.toString() || "0.7",
  );
  ...
  useEffect(() => {
    const saved =
      typeof nodeData.llmPrompt === "string" ? nodeData.llmPrompt : "";
      ...
      <div>
        <div className="mb-2 flex items-center justify-between">
          ...
          <VariableInsertButton
            ...
            onChange={(value) => {
              ...
              onUpdate(node.id, {
                ...nodeData,
                llmSystemPrompt: value || undefined,
              });
            }}
          />
        </div>
        <Textarea
          ...
          onChange={(e) => {
            ...
            onUpdate(node.id, {
              ...nodeData,
              llmSystemPrompt: e.target.value || undefined,
            });
          }}
...
function ToolNodeConfig({
  ...
  const [toolName, setToolName] = useState(
    nodeData.toolName || nodeData.tool_name || "",
  );
...
function LoopNodeConfig({
  ...
  const [pendingItemsVariableName, setPendingItemsVariableName] = useState(
    nodeData.pendingItemsVariableName ||
      nodeData.pending_items_variable_name ||
      "pending_items",
  );
  const [showLoopBodySelector, setShowLoopBodySelector] = useState<
    number | null
  >(null); // 显示选择器的条件索引
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        />
        <p className="text-muted-foreground mt-1 text-xs">
          用于在循环体内访问待优化数据的变量名，可在LLM节点的Prompt中使用 {"{"}
          {"{"}loop.variables.{pendingItemsVariableName}
          {"}"}
          {"}"} 引用
        </p>
      </div>
