...
export function LoopNode({
  ...
  return (
    ...
    >
      ...
      <div className="border-border/50 bg-card/80 pointer-events-none absolute top-0 right-0 left-0 z-10 flex h-10 items-center gap-1.5 rounded-t-lg border-b px-2 backdrop-blur-sm">
        <div className="pointer-events-auto flex h-5 w-5 items-center justify-center rounded bg-orange-100 dark:bg-orange-900/30">
          ...
          <div className="text-muted-foreground truncate text-[10px]">
            {loopCount
              ...
              : breakConditions.length > 0
                ? `${breakConditions.length}个条件`
                : "未配置"}{" "}
            · {loopBodyNodeCount} 个节点
          </div>
        </div>
      </div>
      ...
      {/* 循环体内容区域 - 完全透明，不遮挡子节点 */}
      <div
        className="absolute top-10 right-0 bottom-0 left-0 rounded-b-lg"
        style={{
          width: "100%",
      ...
      >
        ...
        {data.bodyNodesCount === 0 && (
          <div className="text-muted-foreground pointer-events-none p-4 text-xs">
            将节点拖入此区域以添加到循环体
          </div>
        )}
      </div>
      ...
      >
        ...
        >
          <div
            className="absolute right-[1px] bottom-[1px] cursor-nwse-resize transition-opacity hover:opacity-100"
            style={{
              width: "16px",
              height: "16px",
      ...
      <Handle
        ...
        className="!bg-muted-foreground !border-card !top-1/2 z-30 !h-2.5 !w-2.5 !cursor-crosshair !border-2"
        style={{ top: "50%" }}
      />
      <Handle
        type="source"
