...
export function MoleculeOptimizationHistory({
  ...
  const normalizeSmiles = (s: unknown): string =>
  ...
  const getMoleculeIdFromWorkflow = (
    iter: number,
    smiles: string,
  ): {
    id?: number | string;
      ...
      if (Array.isArray(nodeOutput.passed_items)) {
        for (const item of nodeOutput.passed_items) {
          if (
            item &&
            typeof item === "object" &&
            (item.smiles === smiles || item.SMILES === smiles)
          ) {
            return {
              id: item.id,
              previousId:
  ...
  // 根据分子ID查找分子
  const findMoleculeById = (
    iter: number,
    moleculeId: number | string,
  ): Partial<Molecule> | null => {
    const iterOutputs = iterationNodeOutputs.get(iter);
  ...
  const buildEvolutionChain = (
    ...
    for (let i = iterationSnapshots.length - 1; i >= 0; i--) {
      ...
      if (found) {
        ...
        // 从工作流评估节点获取评分和描述
        const evalInfo = getEvaluationFromWorkflow(
          snapshot.iter,
          currentSmiles,
        );
        ...
        if (idInfo.previousId && i > 0) {
          ...
          const prevMolecules = [
            ...prevSnapshot.passed,
            ...prevSnapshot.pending,
          ];

          const bestSmiles = normalizeSmiles(
            (prevSnapshot.best as any)?.smiles ??
              (prevSnapshot.best as any)?.SMILES,
          );
          if (bestSmiles && bestSmiles !== currentSmiles) {
            ...
            const firstPrev = prevMolecules.find((m) => {
              const s = normalizeSmiles(
                (m as any)?.smiles ?? (m as any)?.SMILES,
              );
              return s && s !== currentSmiles;
            });
            if (firstPrev) {
              currentSmiles = normalizeSmiles(
                (firstPrev as any)?.smiles ?? (firstPrev as any)?.SMILES,
              );
            }
          }
        }
  ...
  // 确保与执行日志和趋势图使用相同的数据源
  const getEvaluationFromWorkflow = (
    iter: number,
    smiles: string,
  ): {
    score?: {
    ...
    for (const [nodeId, nodeOutput] of Object.entries(iterOutputs)) {
      ...
      // 优先查找明确标识为总结节点的节点
      if (Array.isArray(nodeOutput.output) && isSummaryNode) {
        for (const item of nodeOutput.output) {
          if (
            item &&
            typeof item === "object" &&
            ...
            if (Array.isArray(iterationOutputs)) {
              ...
              if (typeof promptText === "string" && promptText.length > 0) {
                const dimsById =
                  extractDimScoresFromResolvedInputsPrompt(promptText);
            ...
            if (totalScore > 0 || dimScores) {
              result.score = {
                total:
                  ...
                  (dimScores
                    ? Math.round(
                        (((dimScores.surfaceAnchoring || 0) +
                          (dimScores.energyLevel || 0) +
                          (dimScores.packingDensity || 0)) /
                          3) *
                          10,
                      ) / 10
                    : 0),
    ...
    for (const [nodeId, nodeOutput] of Object.entries(iterOutputs)) {
      ...
      // 识别评估节点（LLM1/2/3）
      if (
        nodeName.includes("llm1") ||
        nodeName.includes("llm2") ||
        nodeName.includes("llm3") ||
            ...
            <div className="space-y-1 text-[10px] text-slate-600 dark:text-slate-400">
              ...
              {entry.description && (
                <div className="mt-1">
                  ...
                  <div className="line-clamp-3 pl-2 text-[9px] text-slate-500 dark:text-slate-400">
                    {entry.description}
                  </div>
                </div>
              )}
