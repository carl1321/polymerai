...
export function ExecutionLogPanel({
  ...
  const getWorkflowName = (): string => {
    if (workflowGraph?.nodes && workflowGraph.nodes.length > 0) {
      // 尝试从节点数据中获取工作流名称
      const startNode = workflowGraph.nodes.find(
        (n: any) => n.type === "start",
      );
  ...
  const generateLogText = (): string => {
    ...
    if (iterationSnapshots.length > 0 || iterationNodeOutputs.size > 0) {
      ...
      if (iterationSnapshots.length > 0) {
        for (const snapshot of iterationSnapshots) {
          ...
          if (iterOutputs && workflowGraph) {
            ...
            const nodeMap = new Map(
              workflowGraph.nodes.map((n: any) => [n.id, n]),
            );
            ...
            for (const [nodeId, nodeOutput] of Object.entries(iterOutputs)) {
              ...
              const hasOutput =
                nodeOutput &&
                (nodeOutput.output !== undefined ||
                  Object.keys(nodeOutput).length > 0);

              if (
                nameLower.includes("生成") ||
                nameLower.includes("generation") ||
                (nodeType === "tool" && Array.isArray(nodeOutput.output))
              ) {
                generationNodes.push({ name: nodeName, output: nodeOutput });
              } else if (
                nameLower.includes("评估") ||
                nameLower.includes("evaluation") ||
                nameLower.includes("llm1") ||
                nameLower.includes("llm2") ||
                nameLower.includes("llm3") ||
                (nodeType === "llm" &&
                  !nameLower.includes("总结") &&
            ...
            if (generationNodes.length > 0) {
              ...
              for (const { name, output } of generationNodes) {
                if (Array.isArray(output.output)) {
                  const molecules = output.output.filter(
                    (item: any) => item?.smiles,
                  );
                  if (molecules.length > 0) {
                    lines.push(
                      `    ${name}: 生成 ${molecules.length} 个候选分子`,
                    );
                    // 只显示前5个分子的SMILES
                ...
                if (output.output !== undefined) {
                  if (typeof output.output === "string") {
                    ...
                    if (outputText.trim().length > 0) {
                      ...
                      // 查找评分信息
                      const scoreMatch = outputText.match(
                        /总分[：:]\s*(\d+\.?\d*)|总评分[：:]\s*(\d+\.?\d*)|score[：:]\s*(\d+\.?\d*)/i,
                      );
                      const dimMatches = [
                      ...
                      if (scoreMatch || dimMatches.some((m) => m)) {
                        ...
                        dimMatches.forEach((match, idx) => {
                          if (match) {
                            ...
                            lines.push(
                              `      ${labels[idx]}: ${parseFloat(match[1]).toFixed(1)}`,
                            );
                          }
                        });
                      }
                      ...
                      // 移除评分相关的行
                      const scorePattern =
                        /(总分|总评分|score|表面锚定|能级匹配|膜致密度)[：:]\s*\d+\.?\d*/gi;
                      ...
                      // 如果还有内容，显示描述
                      const descLines = descriptionText
                        .split("\n")
                        .filter((l: string) => {
                    ...
                    // 数组格式：每个元素是分子的评估结果
                    lines.push(
                      `    ${name}: 评估了 ${output.output.length} 个分子`,
                    );
                    output.output.forEach((item: any, idx: number) => {
                      if (item && typeof item === "object") {
                        ...
                        if (item.opt_des && typeof item.opt_des === "string") {
                          ...
                          if (dimScores) {
                            lines.push(
                              `        表面锚定强度: ${dimScores.surfaceAnchoring.toFixed(1)}`,
                            );
                            lines.push(
                              `        能级匹配: ${dimScores.energyLevel.toFixed(1)}`,
                            );
                            lines.push(
                              `        膜致密度: ${dimScores.packingDensity.toFixed(1)}`,
                            );
                          }
                        }
                        // 显示描述
                        if (
                          ...
                          item.opt_des &&
                          typeof item.opt_des === "string"
                        ) {
                          // 如果没有description，使用opt_des作为描述
                          lines.push(`        评估说明:`);
                          const descLines = item.opt_des
                            .split("\n")
                            .filter((l: string) => l.trim());
                          descLines.forEach((line: string) => {
                            lines.push(`          ${line.trim()}`);
                    ...
                    if (output.output.description) {
                      lines.push(`      评估说明:`);
                      const descLines = output.output.description
                        .split("\n")
                        .filter((l: string) => l.trim());
                      descLines.forEach((line: string) => {
                        lines.push(`        ${line.trim()}`);
                    ...
                    lines.push(
                      `    ${name}: ${JSON.stringify(output.output).substring(0, 100)}...`,
                    );
                  }
                } else {
                  lines.push(`    ${name}: (无输出)`);
                ...
                if (output.output !== undefined) {
                  if (typeof output.output === "string") {
                    ...
                    if (summaryText.length > 0) {
                      ...
                      // 提取维度评分信息
                      const scoreMatch = summaryText.match(
                        /总分[：:]\s*(\d+\.?\d*)|总评分[：:]\s*(\d+\.?\d*)|score[：:]\s*(\d+\.?\d*)/i,
                      );
                      const dimMatches = [
                        summaryText.match(
                      ...
                      // 提取性质预测信息（HOMO/LUMO等）
                      const homoMatch = summaryText.match(
                        /HOMO[：:]\s*([-]?\d+\.?\d*)/i,
                      );
                      const lumoMatch = summaryText.match(
                        /LUMO[：:]\s*([-]?\d+\.?\d*)/i,
                      );
                      const dipoleMatch = summaryText.match(
                        /偶极矩[：:]\s*(\d+\.?\d*)/i,
                      );
                      ...
                      if (homoMatch || lumoMatch || dipoleMatch) {
                        ...
                        if (homoMatch) {
                          lines.push(
                            `        HOMO: ${parseFloat(homoMatch[1]).toFixed(3)} eV`,
                          );
                        } else {
                        ...
                        if (lumoMatch) {
                          lines.push(
                            `        LUMO: ${parseFloat(lumoMatch[1]).toFixed(3)} eV`,
                          );
                        } else if (!homoMatch) {
                        ...
                        if (dipoleMatch) {
                          lines.push(
                            `        偶极矩: ${parseFloat(dipoleMatch[1]).toFixed(3)} Debye`,
                          );
                        }
                      } else {
                        // 如果没有找到性质预测，检查是否有相关提示
                        if (
                          summaryText.includes("能级") ||
                          summaryText.includes("HOMO") ||
                          summaryText.includes("LUMO")
                        ) {
                          lines.push(
                            `      性质预测: 缺少能级数据（HOMO/LUMO）`,
                          );
                        }
                      }
                    ...
                    output.output.forEach((item: any, idx: number) => {
                      if (item && typeof item === "object") {
                        ...
                        if (item.score !== undefined) {
                          lines.push(
                            `        总分: ${typeof item.score === "number" ? item.score.toFixed(1) : item.score}`,
                          );
                        }
                        if (
                          ...
                          if (item.surfaceAnchoring !== undefined) {
                            lines.push(
                              `          表面锚定强度: ${item.surfaceAnchoring.toFixed(1)}`,
                            );
                          } else {
                            lines.push(
                              `          表面锚定强度: 缺少表面锚定强度评分数据`,
                          ...
                          if (item.energyLevel !== undefined) {
                            lines.push(
                              `          能级匹配: ${item.energyLevel.toFixed(1)}`,
                            );
                          } else {
                            lines.push(
                              `          能级匹配: 缺少能级数据（HOMO/LUMO）`,
                            );
                          }
                          if (item.packingDensity !== undefined) {
                            lines.push(
                              `          膜致密度: ${item.packingDensity.toFixed(1)}`,
                            );
                          } else {
                            lines.push(
                              `          膜致密度: 缺少膜致密度评分数据`,
                        ...
                        if (
                          ...
                          if (item.HOMO !== undefined) {
                            lines.push(
                              `          HOMO: ${item.HOMO.toFixed(3)} eV`,
                            );
                          } else {
                            lines.push(
                              `          HOMO: 缺少能级数据（HOMO/LUMO）`,
                            );
                          }
                          if (item.LUMO !== undefined) {
                            lines.push(
                              `          LUMO: ${item.LUMO.toFixed(3)} eV`,
                            );
                          } else if (item.HOMO === undefined) {
                            lines.push(
                              `          LUMO: 缺少能级数据（HOMO/LUMO）`,
                            );
                          }
                        }
                      } else {
                    ...
                    // 显示维度评分
                    if (
                      output.output.score !== undefined ||
                      output.output.surfaceAnchoring !== undefined ||
                      output.output.energyLevel !== undefined ||
                      output.output.packingDensity !== undefined
                    ) {
                      lines.push(`      维度评分:`);
                      if (output.output.score !== undefined) {
                        lines.push(`        总分: ${output.output.score}`);
                      ...
                      if (output.output.surfaceAnchoring !== undefined) {
                        lines.push(
                          `        表面锚定强度: ${output.output.surfaceAnchoring.toFixed(1)}`,
                        );
                      } else {
                        lines.push(
                          `        表面锚定强度: 缺少表面锚定强度评分数据`,
                      ...
                      if (output.output.energyLevel !== undefined) {
                        lines.push(
                          `        能级匹配: ${output.output.energyLevel.toFixed(1)}`,
                        );
                      } else {
                        lines.push(
                          `        能级匹配: 缺少能级数据（HOMO/LUMO）`,
                      ...
                      if (output.output.packingDensity !== undefined) {
                        lines.push(
                          `        膜致密度: ${output.output.packingDensity.toFixed(1)}`,
                        );
                      } else {
                    ...
                    // 显示性质预测
                    if (
                      output.output.HOMO !== undefined ||
                      output.output.LUMO !== undefined ||
                      output.output.dipole !== undefined
                    ) {
                      lines.push(`      性质预测:`);
                      if (output.output.HOMO !== undefined) {
                        lines.push(
                          `        HOMO: ${output.output.HOMO.toFixed(3)} eV`,
                        );
                      } else {
                        lines.push(`        HOMO: 缺少能级数据（HOMO/LUMO）`);
                      }
                      if (output.output.LUMO !== undefined) {
                        lines.push(
                          `        LUMO: ${output.output.LUMO.toFixed(3)} eV`,
                        );
                      ...
                      if (output.output.dipole !== undefined) {
                        lines.push(
                          `        偶极矩: ${output.output.dipole.toFixed(3)} Debye`,
                        );
                      }
                    }
                    // 显示总结说明
                    if (output.output.summary || output.output.description) {
                      ...
                      const summaryContent =
                        output.output.summary || output.output.description;
          ...
          if (currentSummaryNodes.length > 0) {
            for (const { output } of currentSummaryNodes) {
              if (output.output !== undefined) {
                const candidates: Array<{ smiles: string; score: number }> = [];
                ...
                if (candidates.length > 0) {
                  ...
                  if (Array.isArray(output.output)) {
                    const bestItem = output.output.find(
                      (item: any) =>
                    ...
                    if (bestItem) {
                      bestMolecule = {
                        ...
                        analysis:
                          bestItem.description || bestItem.opt_des
                            ? {
                                description:
                    ...
                    // 字符串格式：需要从文本中提取更多信息
                    const summaryText = output.output;
                    const smilesIndex = summaryText.indexOf(
                      bestCandidate.smiles,
                    );
                    if (smilesIndex >= 0) {
                      const contextStart = Math.max(0, smilesIndex - 100);
                      const contextEnd = Math.min(
                        summaryText.length,
                        smilesIndex + bestCandidate.smiles.length + 500,
                      );
                      const contextText = summaryText.substring(
                        contextStart,
                        contextEnd,
          ...
          if (bestMolecule) {
            ...
            if (bestMolecule.score) {
              ...
              if (bestMolecule.score.surfaceAnchoring !== undefined) {
                lines.push(
                  `    表面锚定强度: ${bestMolecule.score.surfaceAnchoring.toFixed(1)}`,
                );
              }
              if (bestMolecule.score.energyLevel !== undefined) {
                lines.push(
                  `    能级匹配: ${bestMolecule.score.energyLevel.toFixed(1)}`,
                );
              }
              if (bestMolecule.score.packingDensity !== undefined) {
                lines.push(
                  `    膜致密度: ${bestMolecule.score.packingDensity.toFixed(1)}`,
                );
              }
            }
        ...
        const sortedIterations = Array.from(iterationNodeOutputs.keys()).sort(
          (a, b) => a - b,
        );
            ...
            for (const [nodeId, nodeOutput] of Object.entries(iterOutputs)) {
              ...
              if (nodeType === "tool" || nodeType === "llm") {
                if (nodeOutput.output !== undefined) {
                  if (Array.isArray(nodeOutput.output)) {
                    const molecules = nodeOutput.output.filter(
                      (item: any) => item?.smiles,
                    );
                    if (molecules.length > 0) {
                      lines.push(
                    ...
                    (nodeOutput.output.smiles || nodeOutput.output.SMILES)
                  ) {
                    ...
                    lines.push(`\n  【${nodeName}】`);
                    const outputLines = nodeOutput.output
                      .split("\n")
                      .filter((l: string) => l.trim())
                      .slice(0, 5);
            ...
            if (output.output !== undefined) {
              if (Array.isArray(output.output)) {
                ...
                output.output.forEach((item: any, idx: number) => {
                  if (item && typeof item === "object" && item.smiles) {
                    const scoreText = item.score
                      ...
                      : "未评分";
                    lines.push(
                      `  分子 ${idx + 1}: ${item.smiles.substring(0, 50)}... [${scoreText}]`,
                    );
                  }
                });
              } else if (typeof output.output === "string") {
                ...
                if (output.output.split("\n").length > 10) {
                  lines.push(
                    `  ... (还有 ${output.output.split("\n").length - 10} 行)`,
                  );
                }
              } else {
                const outputStr = JSON.stringify(output.output).substring(
                  0,
                  200,
                );
                lines.push(
    ...
    if (molecules.length > 0) {
      ...
      topK.forEach((mol, idx) => {
        ...
        if (mol.score) {
          ...
          if (
            mol.score.surfaceAnchoring !== undefined ||
            ...
            if (mol.score.surfaceAnchoring !== undefined) {
              lines.push(
                `    表面锚定强度: ${mol.score.surfaceAnchoring.toFixed(1)}`,
              );
            }
            ...
            if (mol.score.packingDensity !== undefined) {
              lines.push(
                `    膜致密度: ${mol.score.packingDensity.toFixed(1)}`,
              );
            }
          }
        }
        if (mol.analysis?.description) {
          ...
          if (desc.length > 0) {
            lines.push(
              `  评估说明: ${desc.substring(0, 150)}${desc.length > 150 ? "..." : ""}`,
            );
          }
        }
      });
  ...
  return (
    <div className="flex h-full flex-col bg-white dark:bg-slate-900">
            ...
            <Collapsible open={showDebugLogs} onOpenChange={setShowDebugLogs}>
              <CollapsibleTrigger asChild>
                <Button variant="ghost" size="sm" className="h-6 text-xs">
                  调试日志
                  <ChevronDown
                    className={`ml-1 h-3 w-3 transition-transform ${showDebugLogs ? "rotate-180" : ""}`}
                  />
                </Button>
              </CollapsibleTrigger>
            </Collapsible>
      ...
      <div className="flex-1 overflow-auto p-4">
        <pre
          ref={logRef}
          className="font-mono text-xs leading-relaxed break-words whitespace-pre-wrap text-slate-900 dark:text-slate-100"
        >
          {generateLogText()}
        </pre>
        {logLines.length > 0 && (
          <Collapsible open={showDebugLogs} onOpenChange={setShowDebugLogs}>
            <CollapsibleContent className="mt-2">
              <div className="rounded-md bg-slate-50 p-2 dark:bg-slate-800">
                ...
                <pre className="font-mono text-[10px] leading-relaxed break-words whitespace-pre-wrap text-slate-600 dark:text-slate-400">
