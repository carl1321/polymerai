...
export default function ToolboxSkillPage() {
  ...
  const { mutateAsync: updateMetadata, isPending: isSaving } =
    useUpdateSkillMetadata();
  const skill = skills.find(
    (s) => (s.id && s.id === id) || s.name === decodeURIComponent(id),
  );
  ...
  const handleStartChatWithSkill = () => {
    if (!skill) return;
    router.push(
      `/workspace/chats/new?mode=skill&skill_name=${encodeURIComponent(skill.name)}`,
    );
  };

  if (!skill) {
    return (
      <div className="flex min-h-full flex-col items-center justify-center gap-4 bg-[#F5F5F5] p-6 dark:bg-slate-900">
        <p className="text-slate-600 dark:text-slate-400">
          {isLoading ? "加载中..." : "未找到该技能"}
        </p>
  ...
  return (
    <div className="flex min-h-full flex-col bg-[#F5F5F5] p-6 dark:bg-slate-900">
      <div className="rounded-xl border border-slate-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-800">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2 text-xs text-slate-500">
          <span>
            分类：{(skill.group_name || skill.group || "").trim() || "通用"}
          </span>
          <span>
            可见性：{(skill.visibility || "user") === "org" ? "公有" : "私有"}
          </span>
        ...
        <div className="mt-6 flex items-end gap-3">
          ...
          <Button
            ...
            disabled={!skill.id || isSaving}
          >
            {isSaving ? "保存中..." : "保存分类"}
          </Button>
        </div>
        <div className="mt-4">
          <Button onClick={handleStartChatWithSkill}>用该技能发起对话</Button>
        </div>
      </div>
    </div>
  );
