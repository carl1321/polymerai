...
export default function AgentChatPage() {
  ...
  const { agent_id: agentIdParam } = useParams<{
    agent_id: string;
  }>();

  const agent_id = agentIdParam ?? "";

  const { agent } = useAgent(agent_id);
  ...
  const {
    ...
    context: { ...settings.context, agent_id },
    isMock,
    onSend: () => {
    ...
    onStart: (createdThreadId) => {
      ...
      history.replaceState(
        null,
        "",
        `/workspace/agents/${agent_id}/chats/${createdThreadId}`,
      );
    },
    onFinish: (state) => {
  ...
  const handleSubmit = useCallback(
    (message: PromptInputMessage) => {
      const sendPromise = sendMessage(threadId, message, { agent_id });
      if (message.files.length > 0) {
        return sendPromise;
      }
      void sendPromise;
    },
    [sendMessage, threadId, agent_id],
  );
            ...
            <div className="mr-4 flex items-center">
              <Tooltip content={t.agents.newChat}>
                <Button
                  ...
                  variant="secondary"
                  onClick={() => {
                    router.push(`/workspace/agents/${agent_id}/chats/new`);
                  }}
                >
