...
interface SystemReminderGroup extends GenericMessageGroup<"system-reminder"> {}

interface AsyncTaskProgressGroup extends GenericMessageGroup<"async-task-progress"> {}

export const ASYNC_TASK_REMINDER_MESSAGE_NAME = "async_task_reminder";
export const ASYNC_TASK_PROGRESS_MESSAGE_NAME = "async_task_progress";

export function createAsyncTaskReminderMessage(
  id: string,
  ...
  return {
    id,
    type: "ai",
    name: ASYNC_TASK_REMINDER_MESSAGE_NAME,
    content: markdownContent,
  } as Message;
}

export function createAsyncTaskProgressMessage(
  id: string,
  markdownContent: string,
): Message {
  return {
    id,
    type: "ai",
    name: ASYNC_TASK_PROGRESS_MESSAGE_NAME,
    content: markdownContent,
  } as Message;
}
...
export function getMessageGroups(messages: Message[]): MessageGroup[] {
  ...
  function lastOpenGroup() {
    ...
    if (
      ...
      last.type !== "system-reminder" &&
      last.type !== "async-task-progress"
    ) {
      return last;
    }
    return null;
  ...
  for (const message of messages) {
    ...
    if (
      message.type === "ai" &&
      message.name === ASYNC_TASK_REMINDER_MESSAGE_NAME
    ) {
      groups.push({
        id: message.id,
    ...
    if (
      message.type === "ai" &&
      message.name === ASYNC_TASK_PROGRESS_MESSAGE_NAME
    ) {
      groups.push({
        id: message.id,
        type: "async-task-progress",
        messages: [message],
      });
      continue;
    }
...
export function extractContentFromMessage(message: Message) {
  if (typeof message.content === "string") {
    return normalizeRelativeOutputImagePaths(
      splitInlineReasoningFromAIMessage(message)?.content ??
        message.content.trim(),
    );
  }
...
export function extractURLFromImageURLContent(
  ...
  const raw = typeof content === "string" ? content : content.url;
  return normalizeRelativeOutputImagePath(raw);
}

const RELATIVE_OUTPUT_IMAGE_RE =
  /^[^/\s][^/\n]*\.(png|jpg|jpeg|webp|svg|gif|bmp)$/i;

function normalizeRelativeOutputImagePath(path: string): string {
  ...
  if (!RELATIVE_OUTPUT_IMAGE_RE.test(trimmed)) {
    return path;
  }
  return `/mnt/user-data/outputs/${trimmed}`;
}

function normalizeRelativeOutputImagePaths(content: string): string {
  return content.replace(
    /(<img\b[^>]*\bsrc\s*=\s*["'])([^"']+)(["'])/gi,
    (_, prefix: string, rawPath: string, suffix: string) =>
      `${prefix}${normalizeRelativeOutputImagePath(rawPath)}${suffix}`,
  );
}
