...
const roleSchema = z.object({ code: z.string() }).passthrough();

const extensionUserSchema = z.object({
  id: z.string(),
  email: z.string().email(),
...
function hasAdminRole(roles: Array<{ code?: string }>): boolean {
  return roles.some((r) => r.code === "admin");
}

export const userSchema = z
  .union([legacyUserSchema, extensionUserSchema])
  .transform((u) => {
    if ("system_role" in u) {
      return u;
    }
    const isAdmin = u.is_superuser || hasAdminRole(u.roles ?? []);
    return {
      id: u.id,
      email: u.email,
