...
const config = {
  ...
  devIndicators: false,
  // Dev via nginx (localhost:2026): allow HMR + client navigations from proxy origin (Next 15.2.2+).
  allowedDevOrigins: [
    "localhost:2026",
    "127.0.0.1:2026",
    `localhost:${process.env.PORT || "18083"}`,
    `127.0.0.1:${process.env.PORT || "18083"}`,
  ],
  async rewrites() {
    const rewrites = [];
