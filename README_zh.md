...
4. **启动服务**：
   ```bash
   make dev                    # 前端与网关各占端口（详见 `make help`）
   make dev-with-nginx         # 统一入口：http://localhost:2026（需要单一地址时推荐）
   ```

5. **访问地址**：
   - 使用 `make dev-with-nginx`：http://localhost:2026
   - 仅 `make dev`：前端与网关端口以终端输出为准（默认 `18083` / `18084`，可用 `DEER_FLOW_FRONTEND_PORT`、`DEER_FLOW_GATEWAY_PORT` 覆盖）。

   **本地生产模式预览**（关闭热更新）：`make start`。若也要走本机 Nginx（2026 统一入口，与 `dev-with-nginx` 一致），请执行 `make start-with-nginx`，或 `DEER_FLOW_USE_NGINX=1 make start`。

### 进阶配置

#### 公开智能体（免登录链接）

在已配置 **应用数据库**（`app_database.url`）且从工作区 **登录** 后，可在「智能体」管理卡片上点击 **公开**，生成形如 `https://<你的域名>/a/<slug>?token=<secret>` 的链接。访客无需登录即可与对应智能体对话；网关会校验 token，并限制线程枚举、删除及无状态 `runs/*` 等危险路径。LangGraph 进程地址由网关环境变量 **`LANGGRAPH_INTERNAL_URL`** 指定（默认 `http://127.0.0.1:2024`）。使用 `make dev-with-nginx` / `make start-with-nginx` 时，Nginx 已将 `/api/public` 转发到 Gateway。
...
**工作流工具库**（需配置 `app_database`）：在 Web 端 **工作流工具库**（`/workspace/workflow-tools`）中编写 LangChain `@tool` 脚本、配置 `requirements`、试运行并发布；工作流编辑器的 Tool 节点仅从已发布目录中选择工具并填写参数（不支持在节点内改脚本）。自定义工具与内置/MCP 工具统一通过 `ainvoke` 执行；依赖默认安装到 `backend/.deer-flow/.venv`（清华源），若 pip 解析冲突则取消安装且不改动现有环境。
